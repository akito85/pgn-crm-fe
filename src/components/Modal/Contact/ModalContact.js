...
const ModalContact = ({
  ...
  const handleChooseContact = useCallback(
    (r) => {
  ...
  const columns = useMemo(() => {
    return [
      {
        ...
        render: (text, object, index) =>
          (pageContact - 1) * pageContactSize + index + 1,
      },
      {
        title: "TYPE",
        dataIndex: "type",
    ...
    datas_option?.data_contact_type,
    datas_option?.data_country_code,
    datas_option?.data_country_zone,
    datas_option?.data_input_type,
    ...
    (record, formValue, value, prefix) => {
      if (prefix[1] === "prefix_1") {
        ...
        hasValue(value?.value) &&
          dispatch(dispatcher?.dispatcherCountryZone(value?.value));
      } else {
        formContact?.resetFields([
          ["values", 0, "sufix"],
          ["values", 0, "value"],
