...
const DynamicTableInlineWithoutBase = ({
  onDataChange,
  cols,
  tableData,
  mode,
  ...
  const edit = (record, field) => {
    ...
    setTempUpdate({
      ...record,
      suffix: suffix[`${keyModal}~${record.key}`],
      prefix1: prefix1[`${keyModal}~${record.key}`],
      prefix2: prefix2[`${keyModal}~${record.key}`],
      value: value[`${keyModal}~${record.key}`],
    });
  ...
  const cancel = (key) => {
    if (statusAction === "add") {
      ...
      setValue((prevState) => {
        return {
          ...prevState,
          [`${keyModal}~${key}`]: tempUpdate?.value, // Reset the value for changes
        };
      });
    }
    setEditingKey("");
  ...
  const deleteRow = (key) => {
    ...
    setValue((prevState) => {
      let temp = { ...prevState };
      // console.log(temp[`${keyModal}~${key}`], "temp delete value Key Modal and key")
      delete temp[`${keyModal}~${key}`];
      // Iterate over the remaining keys and update their values
      Object.keys(temp).forEach((existingKey) => {
        ...
        if (newKeyNumber > key && parseInt(existingKeyModal) === keyModal) {
          const newKey = `${existingKeyModal}~${newKeyNumber - 1}`;
          temp[newKey] = temp[existingKey];
          // console.log(temp[existingKey], "temp existingKey")
          // console.log(temp[newKey], "temp newKey")
          delete temp[existingKey];
        }
      });
...
export default DynamicTableInlineWithoutBase;
