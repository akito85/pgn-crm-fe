...
const EditableCell = ({
    ...
    // render node
    const getInputNode = useCallback((dataIndex, options, form, rules) => {
        ...
        const maxLengthValidation = () => {
            ...
            } else if (formValue?.inputType?.value === 748 || formValue?.inputType?.value === 751) {
        ...
        if (dataIndex === 'type' || dataIndex === 'inputType') {
            return (
                <Form.Item
                    name={dataIndex}
                    rules={rules}>
                    <SelectComponent options={options} onChange={onCellClicked} labelInValue />
                </Form.Item>
            )
        } else if (dataIndex === 'value') {
            if (formValue?.inputType?.value === 749) {
                return (
                    <Form.List name={'values'} initialValue={[{ prefix_1: undefined, value: undefined }]}>
                        {(fields) => {
                            return (
                                <>
            ...
            //         </Form.List>
            //     )
            } else if (formValue?.inputType?.value === 751 || formValue?.inputType?.value === 748) {
                return (
                    <Form.List name={'values'} initialValue={[{ prefix_1: undefined, value: undefined, prefix_2: undefined, sufix: undefined }]}>
                        {(fields) => {
                            return (
                                <>
                                    {fields?.map(({ key, name }, indexList) => {
                                        return (
                                            <div className='flex gap-2 w-full'>
                                                <Form.Item
                                                    ...
                                                    className='w-1/3'
                                                >
                                                    <SelectComponent labelInValue options={options?.country_code} placeholder={"Country Code"} onChange={(e) => {
                                                        changePrefix(record, formValue, e, [name, 'prefix_1'])
                                                        setLengthPrefix_1(e?.label?.split(' ')?.at(-1)?.length)
                                                    }}
                                                    />
                                                ...
                                                <Form.Item
                                                    ...
                                                    className='w-1/3'
                                                >
                                                    <SelectComponent placeholder={"Zone Code"} labelInValue
                                                        onChange={(e) => {
                                                            changePrefix(record, formValue, e, [name, 'prefix_2'])
                                                            setLengthPrefix_2(e?.label?.split(' ')?.at(0)?.length)
                                                        }} options={options?.country_zone} />
                                        ...
                                        return (
                                            <>
                                                <Form.Item
                                                    name={[name, 'value']}
                                                    rules={[
                                                        ...formMessageRequired(formValue?.inputType?.value === 750 ? 'Email' : 'Value'),
    ...
    if (dataIndex === "action" ||
        ...
        return (
            <td {...restProps}>
                {editing ? (
                    getInputNode(dataIndex, options, form, rules)
                ) : (
                    children
                )}
            </td>
...
const TableContact = ({
    ...
    const setFullValue = useCallback((row) => {
        ...
        let prefix_1 = hasValue(row?.values[0]?.prefix_1?.label) ? row?.values[0]?.prefix_1?.label : '';
        let prefix_2 = hasValue(row?.values[0]?.prefix_2?.label) ? row?.values[0]?.prefix_2?.label : '';
        let sufix = hasValue(row?.values[0]?.sufix) ? `Ext. ${row?.values[0]?.sufix}` : '';
        ...
        try {
            ...
            if (index > -1) {
                ...
                const updatedRow = {
                    ...item, ...{
                        ...row,
                        value: hasValue(row?.values[0]?.value) ? row?.values[0]?.value : null,
                        prefix_1: hasValue(row?.values[0]?.prefix_1) ? row?.values[0]?.prefix_1 : null,
                        prefix_2: hasValue(row?.values[0]?.prefix_2) ? row?.values[0]?.prefix_2 : null,
                        sufix: hasValue(row?.values[0]?.sufix) ? row?.values[0]?.sufix : null,
    ...
    const columnAction = useContactHooks(editingKey, handleDetail, handleUpdate, handleInactivate, handleDelete, handleCancel, handleSave, actionButtons)
    ...
    const handleFilterColumn = useCallback((columns) => {
        const filteredColumn = columns?.filter(item => (
            !filterColumn?.includes(item?.title)
        ))
        ...
        <div className={"w-full flex flex-col gap-4"}>
            ...
            <Form form={form}>
                <Table
                    ...
                    columns={
                        handleFilterColumn(columns?.map(item => ({
                            ...item,
                            onCell: (record) => ({
