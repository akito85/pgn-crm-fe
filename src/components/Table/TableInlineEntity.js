    ...
    switch (inputType) {
      case "text":
        return (
          <Input
            onInput={onInput}
            maxLength={maxLength !== undefined ? maxLength : 255}
          />
        );
      case "input_regex":
        return (
...
const TableInlineEntity = ({
  ...
  const save = async (key) => {
    try {
      ...
      let dataValid = handleValidate(row, statusAction, key);
  ...
  return useContainer === true ? (
    <BaseContainer header={header}>
      <div className={"w-full flex flex-col gap-4"}>
        ...
        <div className={"w-full flex mb-5 gap-2 justify-between"}>
          <span className="font-bold text-red-700">{messageValidate}</span>
        </div>
      </div>
    </BaseContainer>
  ) : (
...
export default TableInlineEntity;
