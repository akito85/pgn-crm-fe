import { Select, Table, Button, Dropdown, Checkbox, Radio, Divider, Input } from "antd";
import React, { useEffect, useState, useMemo } from "react";
import { SettingOutlined, PushpinOutlined, DownOutlined } from "@ant-design/icons";
...
const TablePayment = ({
  ...
  const processedColumns = useMemo(() => {
    ...
    filteredCols.forEach((col) => {
      ...
      const colWithFixed = {
        ...
        // Inject rowIndex into each cell
        onCell: (_, rowIndex) => ({
          recordIndex: rowIndex,
        }),
      };

      if (fixedPos === "left") leftFixed.push(colWithFixed);
      else if (fixedPos === "right") rightFixed.push(colWithFixed);
      else normal.push(colWithFixed);
    });
  ...
  // Menu untuk Show/Hide Column (style TableRBI)
  const showHideMenu = (
    <div
      style={{
        padding: 10,
  ...
  return (
    <div className={"relative flex flex-col w-full"}>
      <div className="flex gap-2 justify-between items-center mb-4">
        {/* Show/Hide Column Button (Style TableRBI) */}
      ...
      <Table
        ...
        // === ADD: This overrides cell background inline, not CSS ===
        components={{
        body: {
          cell: (props) => {
            const isSelected = props.recordIndex === selectedRowIndex;

            const zebraColor =
              props.recordIndex % 2 === 0 ? "#ffffff" : "#e6f0f4";
            ...
            return (
              <td
                ...
                style={{
                  ...props.style,
                  background: isSelected ? "#d7e8ee" : zebraColor,
