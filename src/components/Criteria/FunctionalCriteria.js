      ...
      case "select":
        return (
          <SelectComponent
            ...
            onChange={(e) =>
              handleEditDataRecord(e?.value, key, dataIndex, record)
            }
          >
            {options.map((option) => (
...
const FunctionalCriteria = ({
    ...
    return [
      ...
      ...columnCriteria?.map((item) => ({
        ...
        sorter: (a, b) =>
          sorterFunction(
            item?.dataIndex,
            a[item?.dataIndex]?.label,
            b[item?.dataIndex]?.label,
            item?.inputType,
          ),
        ...
        filteredValue: search?.[item?.dataIndex]
  ...
  const filterColumn = useCallback(
    (dataColumn) => {
      let lowerDefaultColumn = defaultColumn?.map((item) =>
        item?.toLowerCase(),
      );
      let sliceColumn = dataColumn?.filter((item) =>
        lowerDefaultColumn.includes(item?.dataIndex?.toLowerCase()),
      );
      let filteredColumn = dataColumn?.filter((item) =>
      ...
      lowerDefaultColumn.forEach((col) => {
        ...
        let column = sliceColumn.find(
          (item) => item?.dataIndex?.toLowerCase() === col,
        );
        if (column) {
  ...
  const addRow = useCallback(() => {
    ...
    updateStoredData(true);
    setStatusAction("add");
    const filteredColumns = filterColumn(column)?.filter(
      (item) => item?.dataIndex !== "no",
    );
          ...
          >
            <div className="px-5 pt-5 pb-[10px] justify-center">
              ...
              <p className="pl-[70px]">{`You can't create criteria. ${
                dataCriteria?.includes(37) && dataTable?.length > 0
