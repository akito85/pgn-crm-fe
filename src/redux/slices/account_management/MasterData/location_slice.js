...
export const getLocationPaginate = createAsyncThunk("GET_LOCATION_PAGINATE", async ({ typeId, page, pageSize, search, sort }, thunkAPI) => {
    try {
        ...
        // }
        thunkAPI.dispatch(validateError({ error: error, action: "GET_LOCATION_PAGINATE", back: false}))
        return thunkAPI.rejectWithValue(error.response);
    }
});

export const downloadLocation = createAsyncThunk('DOWNLOAD_LOCATION', async ({ parentType, search, page, pageSize, sort }, thunkAPI) => {
    try {
        const searchParams = search === undefined ? "" : search;
        const sortParams =
            sort === undefined || sort === "" ? "createdDate~desc" : sort;
        ...
        thunkAPI.dispatch(validateError({ error: response, action: "DOWNLOAD_LOCATION", back: false }))
        return thunkAPI.rejectWithValue(response.response.data);
    }
});

export const getDetailLocation = createAsyncThunk("GET_DETAIL_LOCATION", async ({ id, locationType }, thunkAPI) => {
    try {
        let url = ``;
...
export const createLocation = createAsyncThunk("CREATE_LOCATION", async (body, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'created', errorMessage(error)), action: "CREATE_LOCATION", back: false }))
        // const message =
        //     (error.response &&
        //         error.response.data &&
...
export const updateLocation = createAsyncThunk("UPDATE_LOCATION", async ({ body, id }, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'created', errorMessage(error)), action: "UPDATE_LOCATION", back: false }))
        // const message =
        //     (error.response &&
        //         error.response.data &&
        //         error.response.data.message) ||
...
export const activationLocation = createAsyncThunk("ACTIVATION_LOCATION",
    async (body, thunkAPI) => {
        try {
            ...
            thunkAPI.dispatch(validateError({ error: error, action: "ACTIVATION_LOCATION", back: false }));
            return thunkAPI.rejectWithValue(error.response.data);
        }
    }
);

export const getLocationReference = createAsyncThunk("GET_LOCATION_REFERENCE", async ({id}, thunkAPI) => {
    try {
...
export const getLocationType = createAsyncThunk("GET_LOCATION_TYPE", async (id, thunkAPI) => {
    try {
...
export const getLocationParent = createAsyncThunk("GET_LOCATION_PARENT", async ({ id, page, pageSize }, thunkAPI) => {
    try {
...
export const getLocationParentType = createAsyncThunk("GET_LOCATION_PARENT_TYPE", async (id, thunkAPI) => {
    try {
...
const locationSlice = createSlice({
    ...
    extraReducers: {
        // pagination
        [getLocationPaginate.pending]: (state, action) => {
            state.loading = true;
        },
        [getLocationPaginate.rejected]: (state, action) => {
            state.loading = false;
        },
        ...
        // download
        [downloadLocation.pending]: (state, action) => {
            state.loading = true;
        },
        [downloadLocation.rejected]: (state, action) => {
            state.loading = false;
        },
        [downloadLocation.fulfilled]: (state, action) => {
            state.loading = false;
        },
        // detail
        [getDetailLocation.pending]: (state, action) => {
            state.loading = true;
        },
        [getDetailLocation.rejected]: (state, action) => {
            state.loading = false;
        },
        ...
        // activation
        [activationLocation.pending]: (state, action) => {
            state.loading = true;
        },
        [activationLocation.rejected]: (state, action) => {
            state.loading = false;
        },
        [activationLocation.fulfilled]: (state, action) => {
            state.loading = false;
            // state.data = action.payload;
        },
        // get location reference
        [getLocationReference.pending]: (state, action) => {
            state.loading = true;
        },
        [getLocationReference.rejected]: (state, action) => {
            state.loading = false;
        },
        ...
        // get location type
        [getLocationType.pending]: (state, action) => {
            state.loading = true;
        },
        [getLocationType.rejected]: (state, action) => {
            state.loading = false;
        },
        ...
        // get location parent
        [getLocationParent.pending]: (state, action) => {
            state.loading = true;
        },
        [getLocationParent.rejected]: (state, action) => {
            state.loading = false;
        },
        ...
        [getLocationParentType.pending]: (state, action) => {
            state.loading = true;
        },
        [getLocationParentType.rejected]: (state, action) => {
            state.loading = false;
        },
