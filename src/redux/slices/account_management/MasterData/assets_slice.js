...
export const getAssetsPaginate = createAsyncThunk("GET_ASSETS_PAGINATE", async ({ page, pageSize, search, sort }, thunkAPI) => {
    try {
        ...
        // }
        thunkAPI.dispatch(validateError({ error: error, action: "GET_ASSETS_PAGINATE", back: false }));
        return thunkAPI.rejectWithValue(error?.response);
    }
});

export const downloadAssets = createAsyncThunk('DOWNLOAD_ASSETS', async ({ search, page, pageSize, sort }, thunkAPI) => {
    try {
        const searchParams = search === undefined ? "" : search;
        const sortParams =
            sort === undefined || sort === "" ? "createdDate~desc" : sort;
        ...
        thunkAPI.dispatch(validateError({ error: response, action: "DOWNLOAD_ASSETS", back: false }));
        return thunkAPI.rejectWithValue(response.response.data);
    }
});

export const getDetailAssets = createAsyncThunk("GET_DETAIL_ASSETS", async (id, thunkAPI) => {
    try {
        const url = `/v1/dbs/api/master/assets/detail-asset/${id}`;
        const response = await accountManagementService.getDetail(url);
        return response?.data;
    } catch (error) {
        thunkAPI.dispatch(validateError({ error: error, action: "GET_DETAIL_ASSETS", back: false }))
        return thunkAPI.rejectWithValue(error.response);
    }
});
export const createAssets = createAsyncThunk("CREATE_ASSETS", async (body , thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'created', errorMessage(error)), action: "CREATE_ASSETS", back: false }))
        return thunkAPI.rejectWithValue(error.response.data);
    }
});
export const updateAsstes = createAsyncThunk("UPDATE_ASSETS", async (body, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'updated', errorMessage(error)), action: "UPDATE_ASSETS", back: false }))
        return thunkAPI.rejectWithValue(error.response.data);
    }
});
export const activationAssets = createAsyncThunk("ACTIVATION_ASSETS",
    async (body, thunkAPI) => {
        try {
            ...
            //     thunkAPI.dispatch(showModalError(errorBody));
            // }
            thunkAPI.dispatch(validateError({ error: error, action: "ACTIVATION_ASSETS", back: false }));
            return thunkAPI.rejectWithValue(error.response.data);
        }
    }
);

export const getListAssetName = createAsyncThunk("LIST_ASSET_NAME", async (thunkAPI) => {
    try {
...
export const getListAssetType = createAsyncThunk("LIST_ASSET_TYPE", async (thunkAPI) => {
    try {
...
export const getListAssetBrand = createAsyncThunk("LIST_ASSET_BRAND", async (thunkAPI) => {
    try {
...
export const getListAssetServiceType = createAsyncThunk("LIST_ASSET_SERVICE_TYPE", async (thunkAPI) => {
    try {
...
export const getListProductName = createAsyncThunk("LIST_PRODUCT_NAME", async (thunkAPI) => {
    try {
...
export const getListServicePointName = createAsyncThunk("LIST_SERVICE_POINT_NAME", async (thunkAPI) => {
    try {
        const url = '/v1/dbs/api/master/assets/assetServiceType';
        const response = await accountManagementService.getAll(url);
        return response.data;
...
export const getListGsize = createAsyncThunk("LIST_G_SIZE", async (thunkAPI) => {
    try {
