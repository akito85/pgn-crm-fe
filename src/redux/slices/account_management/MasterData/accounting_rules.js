...
export const getAccountingRulesPaginate = createAsyncThunk("GET_ACCOUNTING_RULES_PAGINATE", async ({ page, pageSize, search, sort }, thunkAPI) => {
    try {
        ...
        // }
        thunkAPI.dispatch(validateError({ error: error, action: "GET_ACCOUNTING_RULES_PAGINATE", back: false }))
        return thunkAPI.rejectWithValue(error?.response);
    }
});

export const downloadAccountingRules = createAsyncThunk('DOWNLOAD_ACCOUNTING_RULES', async ({ search, page, pageSize, sort }, thunkAPI) => {
    try {
        const searchParams = search === undefined ? "" : search;
        const sortParams =
            sort === undefined || sort === "" ? "createdDate~desc" : sort;
        ...
        thunkAPI.dispatch(validateError({ error: response, action: "DOWNLOAD_ACCOUNTING_RULES", back: false }))
        return thunkAPI.rejectWithValue(response.response.data);
    }
});

export const getDetailAccountingRules = createAsyncThunk("GET_DETAIL_ACCOUNTING_RULES", async (id, thunkAPI) => {
    try {
        ...
        //     thunkAPI.dispatch(showModalError(errorBody))
        // }
        thunkAPI.dispatch(validateError({ error: error, action: 'GET_DETAIL_ACCOUNTING_RULES', back: false }))
        return thunkAPI.rejectWithValue(error.response);
    }
});
export const createAccountingRules = createAsyncThunk("CREATE_ACCOUNTING_RULES", async (body, thunkAPI) => {
    try {
        ...
        //     thunkAPI.dispatch(showModalError(errorBody));
        // }
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'created', errorMessage(error)), action: 'CREATE_ACCOUNTING_RULES', back: false }))
        return thunkAPI.rejectWithValue(error.response.data);
    }
});
export const updateAccountingRules = createAsyncThunk("UPDATE_ACCOUNTING_RULES", async (body, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'updated', errorMessage(error)), action: 'UPDATE_ACCOUNTING_RULES', back: false }))
        return thunkAPI.rejectWithValue(error.response.data);
    }
});

export const activationAccountingRules = createAsyncThunk("ACTIVATION_ACCOUNTING_RULES",
    async (body, thunkAPI) => {
        try {
            ...
            //     thunkAPI.dispatch(showModalError(errorBody));
            // }
            thunkAPI.dispatch(validateError({ error: error, action: 'ACTIVATION_ACCOUNTING_RULES', back: false }));
            return thunkAPI.rejectWithValue(error.response.data);
        }
    }
);
