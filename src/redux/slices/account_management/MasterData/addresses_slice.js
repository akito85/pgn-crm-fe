...
export const getAddressesPaginate = createAsyncThunk("GET_ADDRESS_PAGINATE", async ({ page, pageSize, search, sort }, thunkAPI) => {
    try {
        ...
        // }
        thunkAPI.dispatch(validateError({ error: error, action: 'GET_ADDRESS_PAGINATE', back : false}))
        return thunkAPI.rejectWithValue(error.response);
    }
});

export const downloadAddress = createAsyncThunk('DOWNLOAD_ADDRESS', async ({ search, page, pageSize, sort }, thunkAPI) => {
    try {
        const searchParams = search === undefined ? "" : search;
        const sortParams =
            sort === undefined || sort === "" ? "createdDate~desc" : sort;
        ...
        thunkAPI.dispatch(validateError({ error: response, action: 'DOWNLOAD_ADDRESS', back: false }))
        return thunkAPI.rejectWithValue(response.response.data);
    }
});

export const getDetailAddress = createAsyncThunk("GET_DETAIL_ADDRESS", async (id, thunkAPI) => {
    try {
        const url = `/v1/dbs/api/master/address/detail/${id}`;
        const response = await accountManagementService.getDetail(url);
        return response?.data;
    } catch (error) {
        thunkAPI.dispatch(validateError({ error: error, action: 'GET_DETAIL_ADDRESS', back: false }))
        return thunkAPI.rejectWithValue(error.response);
    }
});
export const createAddress = createAsyncThunk("CREATE_ADDRESS", async (body, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'created', errorMessage(error)), action: 'CREATE_ADDRESS', back: false }))
        return thunkAPI.rejectWithValue(error.response.data);
    }
});
export const updateAddress = createAsyncThunk("UPDATE_ADDRESS", async (body, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'updated', errorMessage(error)), action: 'UPDATE_ADDRESS', back: false }))
        return thunkAPI.rejectWithValue(error.response.data);
    }
});
export const activationAddress = createAsyncThunk("ACTIVATION_ADDRESS",
    async (body, thunkAPI) => {
        try {
            ...
            thunkAPI.dispatch(validateError({ error: error, action: 'ACTIVATION_ADDRESS', back: false }));
            return thunkAPI.rejectWithValue(error.response.data);
        }
    }
);
...
// lov
export const getListCountry = createAsyncThunk("GET_LIST_COUNTRY", async (thunkAPI) => {
    try {
        const url = '/v1/dbs/api/master/address/list-country';
        const response = await accountManagementService.getAll(url);
        const responseMap = response?.data?.map(item => (
            { name: item?.locationName, value: item?.locationId }
        ));
        return responseMap;
    } catch (error) {
        const message =
...
export const getListProvince = createAsyncThunk("GET_LIST_PROVINCE", async (id, thunkAPI) => {
    try {
        if (hasValue(id)) {
...
export const getListCity = createAsyncThunk("GET_LIST_CITY", async (id, thunkAPI) => {
    try {
        if (hasValue(id)) {
...
export const getListDistrict = createAsyncThunk("GET_LIST_DISTRICT", async (id, thunkAPI) => {
    try {
        if (hasValue(id)) {
...
export const getListSubDistrict = createAsyncThunk("GET_LIST_SUB_DISTRICT", async (id, thunkAPI) => {
    try {
        if (hasValue(id)) {
...
export const getListPostalCode = createAsyncThunk("GET_LIST_POSTAL_CODE", async (id, thunkAPI) => {
    try {
        if (hasValue(id)) {
...
export const getTypeAddress = createAsyncThunk("GET_TYPE_ADDRESS", async (thunkAPI) => {
    try {
...
const addressSlice = createSlice({
    ...
    extraReducers: {
        // pagination
        [getAddressesPaginate.pending]: (state, action) => {
            state.loading = true;
        },
        [getAddressesPaginate.rejected]: (state, action) => {
            state.loading = false;
        },
        ...
        // download
        [downloadAddress.pending]: (state, action) => {
            state.loading = true;
        },
        [downloadAddress.rejected]: (state, action) => {
            state.loading = false;
        },
        [downloadAddress.fulfilled]: (state, action) => {
            state.loading = false;
            // state.data = action.payload;
        },
        // detail
        ...
        // activation
        [activationAddress.pending]: (state, action) => {
            state.loading = true;
        },
        [activationAddress.rejected]: (state, action) => {
            state.loading = false;
            // state.data = action.payload;
        },
        [activationAddress.fulfilled]: (state, action) => {
            state.loading = false;
            // state.data = action.payload;
        },
        ...
        // postal code
        [getTypeAddress.pending]: (state, action) => {
            state.loading = true;
        },
...
export const { setClearProvince, setClearCity, setClearDistrict, setClearSubDistrict } = addressSlice.actions;
