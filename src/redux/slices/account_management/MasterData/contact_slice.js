...
export const getContactPaginate = createAsyncThunk("GET_CONTACT_PAGINATE", async ({ page, pageSize, search, sort }, thunkAPI) => {
    try {
        ...
        // }
        thunkAPI.dispatch(validateError({ error: error, action: 'GET_CONTACT_PAGINATE' }))
        return thunkAPI.rejectWithValue(error?.response);
    }
});

export const downloadContact = createAsyncThunk('DOWNLOAD_CONTACT', async ({ search, page, pageSize, sort }, thunkAPI) => {
    try {
        const searchParams = search === undefined ? "" : search;
        const sortParams =
            sort === undefined || sort === "" ? "createdDate~desc" : sort;
        ...
        thunkAPI.dispatch(validateError({ error: response, action: 'DOWNLOAD_CONTACT' }))
        return thunkAPI.rejectWithValue(response.response.data);
    }
});

export const getDetailContact = createAsyncThunk("GET_DETAIL_CONTACT", async (id, thunkAPI) => {
    try {
        ...
        //     thunkAPI.dispatch(showModalError(errorBody))
        // }
        thunkAPI.dispatch(validateError({ error: error, action: 'GET_DETAIL_CONTACT' }))
        return thunkAPI.rejectWithValue(error.response);
    }
});
export const createContact = createAsyncThunk("CREATE_CONTACT", async (body, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'created', errorMessage(error)), action: 'CREATE_CONTACT' }))
        return thunkAPI.rejectWithValue(error.response.data);
    }
});
export const updateContact = createAsyncThunk("UPDATE_CONTACT", async (body, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'updated', errorMessage(error)), action: 'UPDATE_CONTACT' }))
        return thunkAPI.rejectWithValue(error.response.data);
    }
});
export const activationContact = createAsyncThunk("ACTIVATION_CONTACT",
    async (body, thunkAPI) => {
        try {
            ...
            //     thunkAPI.dispatch(showModalError(errorBody));
            // }
            thunkAPI.dispatch(validateError({ error: error, action: 'ACTIVATION_CONTACT' }))
            return thunkAPI.rejectWithValue(error.response.data);
        }
    }
);

export const getListJob = createAsyncThunk("GET_LIST_JOB", async (_, thunkAPI) => {
    try {
        ...
        //     thunkAPI.dispatch(showModalError(errorBody))
        // }
        thunkAPI.dispatch(validateError({ error: error, action: 'GET_LIST_JOB' }))
        return thunkAPI.rejectWithValue(error?.response);
    }
});
export const getListPosition = createAsyncThunk("GET_LIST_POSITION", async (_, thunkAPI) => {
    try {
        ...
        //     thunkAPI.dispatch(showModalError(errorBody))
        // }
        thunkAPI.dispatch(validateError({ error: error, action: 'GET_LIST_POSITION' }))
        return thunkAPI.rejectWithValue(error?.response);
    }
});
...
export const getInputType = createAsyncThunk(
    ...
    async (_, thunkAPI) => {
        try {
            ...
            thunkAPI.dispatch(validateError({ error: error, action: 'GET_INPUT_TYPE' }))
            return thunkAPI.rejectWithValue(error?.response);
        }
    }
);
...
export const getContactType = createAsyncThunk(
    ...
    async (_, thunkAPI) => {
        try {
            ...
            thunkAPI.dispatch(validateError({ error: error, action: 'GET_CONTACT_TYPE' }))
            return thunkAPI.rejectWithValue(error?.response);
        }
    }
);
...
export const getCountryCode = createAsyncThunk(
    ...
    async (_, thunkAPI) => {
        try {
            ...
            thunkAPI.dispatch(validateError({ error: error, action: 'GET_COUNTRY_CODE' }))
            return thunkAPI.rejectWithValue(error?.response);
        }
    }
);
...
export const getCountryZone = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            ...
            thunkAPI.dispatch(validateError({ error: error, action: 'GET_COUNTRY_ZONE' }))
            return thunkAPI.rejectWithValue(error?.response);
        }
    }
);
...
const contactSlice = createSlice({
    ...
    extraReducers: {
        ...
        [getCountryZone.fulfilled]: (state, action) => {
            state.loading = false;
            state.data_country_zone = action.payload;
            const uniqueItems = new Set(state.temp_country_zone?.map(item => item.id));
            const filteredItems = action.payload.filter(item => !uniqueItems.has(item.id));
