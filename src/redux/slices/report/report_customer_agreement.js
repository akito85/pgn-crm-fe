    ...
    endpoints: (builder) => ({
        getCustomerAccountAdvancedFilter: builder.mutation({
            ...
            async queryFn({ page, pageSize, sort, search, body }, api, extraOptions, baseQuery) {
                ...
                const url = `/v1/dbs/api/agreement-report/paging-sa-agreement?searchs=${search}&page=${page}&size=${pageSize}&sort=${sortParams}`;
        ...
        getCustomerAgreementDownload: builder.mutation({
            async queryFn({ page, pageSize, sort, search, body }, api, extraOptions, baseQuery) {
                try {
                    const sortParams = !sort ? 'createdDate~desc' : sort;
                    const url = `/v1/dbs/api/agreement-report/download-sa-agreement?searchs=${search}&page=${page}&size=${pageSize}&sort=${sortParams}`;
...
export const { useGetCustomerAccountAdvancedFilterMutation, useGetCustomerAgreementPaginationQuery, useGetCustomerAgreementDownloadMutation } = reportCustomerAgreementSlice;