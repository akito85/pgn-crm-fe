...
export const getMonitoringSession = createAsyncThunk('GET_MONITORING_SESSION', async ({ page, pageSize }, thunkAPI) => {
    try {
        const url = `/v1/dbs/api/monitoring/show-session?page=${page}&size=${pageSize}`
        const data = await userHttpService.getPagination(url);
        return data?.data;
    } catch (error) {
        thunkAPI?.dispatch(validateError({ error: error, action: 'GET_MONITORING_SESSION', back: false }))
...
const monitoringSessionSlice = createSlice({
    ...
    initialState,
    extraReducers: {
        [getMonitoringSession.pending]: (state, action) => {
            state.loading = true;
        },
        ...
        [getMonitoringSession.rejected]: (state, action) => {
            state.loading = false;
