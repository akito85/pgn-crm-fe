...
export const getPositionHierarchyPaginate = createAsyncThunk("GET_POSITION_HIERARCHY_PAGINATE", async ({ search, page, pageSize, sort }, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: error, action: "GET_POSITION_HIERARCHY_PAGINATE", back: false }));
        return thunkAPI.rejectWithValue([]);
    }
});

// pagination position hierarchy
export const getPosition = createAsyncThunk("GET_POSITION", async (_, thunkAPI) => {
    try {
...
export const getDetailHierarchy = createAsyncThunk("GET_DETAIL_HIERARCHY", async (id, thunkAPI) => {
    try {
        let url = `/v1/dbs/api/positionhierarchy/detail/${id}`;
        const response = await userHttpService.getDetail(url);
        return response?.data;
    } catch (error) {
        thunkAPI.dispatch(validateError({ error: error, action: "GET_DETAIL_HIERARCHY", back: true }));
        return thunkAPI.rejectWithValue(error?.response);
    }
});

// get detail position
export const getDetailPosition = createAsyncThunk("GET_DETAIL_POSITION", async (id, thunkAPI) => {
    try {
        let url = `/v1/dbs/api/positionhierarchy/employee/${id}`;
        const response = await userHttpService.getDetail(url);
        return response?.data;
    } catch (error) {
        thunkAPI.dispatch(validateError({ error: error, action: "GET_DETAIL_POSITION", back: false }));
        return thunkAPI.rejectWithValue(error?.response);
    }
});
...
// create func
export const createPositionHierarchy = createAsyncThunk("CREATE_POSITION_HIERARCHY", async (body, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'created', errorMessage(error)), action: "CREATE_POSITION_HIERARCHY", back: false }))

        // if (statusCode?.includes(error?.response?.data?.code)) {
        //     thunkAPI.dispatch(
        //         validateError({ error: error, action: "CREATE_POSITION_HIERARCHY", back: false })
        //     );
        // } else {
        //     const errorBody = {
        //         title: "Failed",
...
// update func
export const updatePositionHierarchy = createAsyncThunk("UPDATE_POSITION_HIERARCHY", async (body, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'updated', errorMessage(error)), action: "UPDATE_POSITION_HIERARCHY", back: false }))

        // if (statusCode?.includes(error?.response?.data?.code)) {
        //     thunkAPI.dispatch(
        //         validateError({ error: error, action: "UPDATE_POSITION_HIERARCHY", back: false })
        //     );
        // } else {
        //     const errorBody = {
        //         title: "Failed",
...
// duplicate func
export const duplicatePositionHierarchy = createAsyncThunk("DUPLICATE_POSITION_HIERARCHY", async (body, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'duplicated', errorMessage(error)), action: "DUPLICATE_POSITION_HIERARCHY", back: false }))

        // const errorLog = error?.response?.data?.code || error?.response?.status;
        // if (statusCode?.includes(errorLog)) {
        //     thunkAPI.dispatch(
        //         validateError({ error: error, action: "DUPLICATE_POSITION_HIERARCHY", back: false })
        //     );
        // } else {
        //     const errorBody = {
        //         title: "Failed",
...
export const activationPositionHierarchy = createAsyncThunk("ACTIVATION_POSITION_HIERARCHY", async (body, thunkAPI) => {
    try {
        ...
        thunkAPI.dispatch(validateError({ error: errorBody(errorCode(error), 'activated', errorMessage(error)), action: "ACTIVATION_POSITION_HIERARCHY", back: false }))
        // if (statusCode?.includes(error?.response?.status) && hasValue(error?.response?.data) === false) {
        //     thunkAPI.dispatch(
        //         validateError({ error: error, action: "ACTIVATION_POSITION_HIERARCHY", back: false })
        //     );
        // } else {
        //     const errorBody = {
        //         title: "Failed",
...
// download hierarchy
export const downloadPositionHierarchy = createAsyncThunk("DOWNLOAD_POSITION_HIERARCHY", async ({ sort, page, pageSize, search }, thunkAPI) => {
    try {
        const searchParams = search === undefined ? "" : search;
        const sortParams =
            sort === undefined || sort === "" ? "createdDate~desc" : sort;
        ...
        thunkAPI.dispatch(validateError({ error, action: 'DOWNLOAD_POSITION_HIERARCHY', back: false }));
