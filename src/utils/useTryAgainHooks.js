...
export const useTryAgainHooks = (handleTryAgain) => {
  ...
  const handleCancelTryAgain = () => {
    ...
    setModalError(false);
    if (
      errorMessage(bodyError)?.toLowerCase() === "network error" ||
      bodyError?.action === "CHECK_GRANTED_ACCESS" ||
      bodyError?.action === "GET_PROFILE"
    ) {
      dispatch(getProfile());
    ...
    return (
      ...
      >
        <div className="px-5 pt-5 pb-[10px] justify-center">
          ...
          <span className="pl-[70px]">Please Contact Administrator</span>
        </div>
      </ModalError>
    );
  };
