...
const TermOfServiceUpdate = () => {
  ...
  // Data Attribute
  const attributeData = useCallback(
    (data_detail?.attributes || []).map((item) => {
      return {
        id: item.id,
        idTos: item.idTos,
  ...
  // Data Criteria
  const criteriaData = useCallback(
    (data_detail?.criterias || []).map((item) => {
      return {
        id: item.id,
        idTos: item.idTos,
  ...
  // Data Criteria List
  const dataCriteriaList = useCallback(
    (data_detail?.listCriterias || [])
  ...
  const routes = [
    ...
    },
    {
      ...
      breadcrumbName: "Update Terms Of Service",
    },
  ];
  ...
  const handleConfirm = () => {
    ...
    // const filteredCriteria = columnsTableCriteriaTOS().filter(
      ...
      <Spin spinning={loading}>
        ...
        <Form layout="vertical" form={form} onFinish={handleSave}>
          <div className="flex flex-col gap-y-4">
            <NxCardContainer header={"TERM OF SERVICE INFORMATION"}>
...
export default TermOfServiceUpdate;
