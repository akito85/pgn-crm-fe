...
const PricingTable = () => {
  ...
  const toolbarItems = useMemo(() => [
    ...
    },
    {
      action: "Create",
      render: (
        <Link to={PRODUCT_PROMO_ROUTES.CREATE_PRICING}>
          <ButtonComponent
  ...
  const itemsActionView = useMemo(() => [
    {
      ...
      render: (record) => (
        <Tooltip title="Detail">
          <Link to={PRODUCT_PROMO_ROUTES.DETAIL_PRICING} state={{ id: record?.id }}>
            <div className="pt-1">
      ...
      render: (record, data_length) => {
        ...
        return isEditable ? (
          <Link
            ...
            state={{ id: record?.id, statusPricing: record?.status, statusApprovalPricing: record?.statusApproval }}
          >
            {icon}
          </Link>
        ) : icon;
