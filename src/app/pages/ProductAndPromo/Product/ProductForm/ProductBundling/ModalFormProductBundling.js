...
const ModalFormProductBundling = ({
  typeForm = "create",
  dataObj = {},
  dataFormObj = {},
  updateTable = () => { },
  ...
  useEffect(() => {
    if (typeForm === "update" && dataFormObj && dataFormObj.productName) {
      form.setFieldsValue({
        ...
        discountValue: dataFormObj.discountAmount,
        discountType: {
          label: dataFormObj.discountTypeName,
          value: dataFormObj.discountType,
        },
      });
    }
  ...
  const handleSaveModal = (data) => {
    ...
    const obj = {
      ...
      discountTypeName: data.discountType?.label,
    };
    let fixData = [];
    if (typeForm === "create") {
  ...
  return (
    <ModalCustom
      ...
      header={`${typeForm === "update" ? "UPDATE" : "CREATE"} PRODUCT BUNDLING`}
      width={1000}
      type={"confirmation"}
      footer={
          ...
          >
            <div className="grid grid-cols-4 gap-4">
              ...
              <Form.Item
                name={"discountType"}
                rules={[
                  { message: requiredMessage("Discount Type"), required: true },
                ]}
                className="no-margin-form"
                label={"Discount Type"}
              >
                {dataListDiscountType && dataListDiscountType.length > 0 && (
                  <SelectComponent labelInValue>
                    {dataListDiscountType.map((data) => (
...
export default ModalFormProductBundling;
