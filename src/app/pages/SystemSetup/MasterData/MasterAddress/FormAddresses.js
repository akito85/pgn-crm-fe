...
const FormAddresses = ({ type }) => {
    ...
    (data) => {
      if (data) {
        form.setFieldsValue({
          ...
          longitude: hasValue(data?.coordinateInformation?.longtitude)
            ? data?.coordinateInformation?.longtitude
            : undefined,
          latitude: data?.coordinateInformation?.latitude,
          source: data?.coordinateInformation?.source,
        });
        setSelectedLocationCreateNew({
          lat: hasValue(data?.coordinateInformation?.latitude)
            ? parseFloat(data?.coordinateInformation?.latitude)
            : "",
          lng: hasValue(data?.coordinateInformation?.longtitude)
            ? parseFloat(data?.coordinateInformation?.longtitude)
            : "",
        });
  ...
  const getValueForm = useCallback(
    (countryId, provinceId, cityId, districtId, subDistrictId) => {
  ...
  const handleChangeProvince = (e) => {
    ...
    form.resetFields(["cityId", "districtId", "subDistrictId", "postalCodeId"]);
  };

  // handle change city
  const handleChangeCity = (e) => {
    dispatch(getListDistrict(e));
    form.resetFields(["districtId", "subDistrictId", "postalCodeId"]);
  };

  // handle change district
  const handleChangeDistrict = (e) => {
    dispatch(getListSubDistrict(e));
    form.resetFields(["subDistrictId", "postalCodeId"]);
  };

  // handle change sub district
  const handleChangeSubDistrict = (e) => {
    dispatch(getListPostalCode(e));
    form.resetFields(["postalCodeId"]);
  };
  ...
  const onInputUpperCase = (e) => {
    ...
    Object.assign(e.target, {
      value: e.target.value.toUpperCase().trimStart(),
    });
          ...
          <BaseContainer header={"create address information"}>
            <div className="w-full grid grid-cols-3 gap-3">
              <FormItem
                label={"Country"}
                name={"countryId"}
                rules={formMessageRequired("Country")}
              >
                <SelectComponent
              ...
              <FormItem
                label={"Province"}
                name={"provinceId"}
                rules={formMessageRequired("Province")}
              >
                <SelectComponent
                  onChange={(e) => handleChangeProvince(e)}
                  disabled={hasValue(countryId) === false || type === "update"}
                >
              ...
              <FormItem
                label={"City"}
                name={"cityId"}
                rules={formMessageRequired("City")}
              >
                <SelectComponent
                  onChange={(e) => handleChangeCity(e)}
                  disabled={hasValue(provinceId) === false || type === "update"}
                >
                  {data_city?.map((item) => (
                    <Select.Option value={item?.value}>
                      {item?.name}
              ...
              <FormItem
                label={"District"}
                name={"districtId"}
                rules={formMessageRequired("District")}
              >
                <SelectComponent
                  onChange={(e) => handleChangeDistrict(e)}
                  disabled={hasValue(cityId) === false || type === "update"}
                >
              ...
              <FormItem
                label={"Sub District"}
                name={"subDistrictId"}
                rules={formMessageRequired("Sub District")}
              >
                <SelectComponent
                  onChange={(e) => handleChangeSubDistrict(e)}
                  disabled={hasValue(districtId) === false || type === "update"}
                >
              ...
              <FormItem
                label={"Postal Code"}
                name={"postalCodeId"}
                rules={formMessageRequired("Postal Code")}
              >
                <SelectComponent
                  disabled={
                    hasValue(subDistrictId) === false || type === "update"
                  }
                >
              ...
              <FormItem label={"Additional Note"} name={"additionalInfo"}>
                <InputComponent
                  onChange={(e) =>
                    handleInputChange(e.target.value, "additionalInfo")
                  }
                  onInput={onInputUpperCase}
                  disabled={type === "update"}
                />
        ...
        <div>
          ...
          <div className="w-full grid grid-cols-4">
            <DetailText label={"Country"}>
              {
                data_country?.filter(
                  (item) => item?.value === body?.body?.countryId,
                )[0]?.name
              }
            </DetailText>
            <DetailText label={"Province"}>
              {
                data_province?.filter(
                  (item) => item?.value === body?.body?.provinceId,
                )[0]?.name
              }
            </DetailText>
            <DetailText label={"City"}>
              {
                data_city?.filter(
                  (item) => item?.value === body?.body?.cityId,
                )[0]?.name
              }
            </DetailText>
            <DetailText label={"District"}>
              {
                data_district?.filter(
                  (item) => item?.value === body?.body?.districtId,
                )[0]?.name
              }
            </DetailText>
          </div>
          <div className="w-full grid grid-cols-4">
            <DetailText label={"Sub District"}>
              {
                data_sub_district?.filter(
                  (item) => item?.value === body?.body?.subDistrictId,
                )[0]?.name
              }
            </DetailText>
            <DetailText label={"Postal Code"}>
              {
                data_postal_code?.filter(
                  (item) => item?.value === body?.body?.postalCodeId,
                )[0]?.name
              }
            </DetailText>
          ...
          <div className="w-full grid grid-cols-4">
            ...
            <DetailText label={"Type"}>
              {
                data_type?.filter((item) => item?.value === body?.body?.type)[0]
                  ?.name
              }
            </DetailText>
