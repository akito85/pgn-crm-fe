...
const ViewAssets = () => {
  ...
  const handleSaveActivation = async (formValue) => {
    ...
    await dispatch(
      getAssetsPaginate({ page, pageSize, sort, search: searchRequest }),
    ).unwrap();
    form.resetFields();
  };
  ...
  const handleConfirm = () => {
    if (bodyError?.action === "GET_ASSETS_PAGINATE") {
      const searchRequest = encodeURIComponent(JSON.stringify(search));
      dispatch(
        getAssetsPaginate({ search: searchRequest, sort, page, pageSize }),
      );
              ...
              >
                <Link
                  to={
                    ...
                    SYSTEM_SETUP_ROUTES.UPDATE_MASTER_ASSETS
                  }
                  state={
                    record?.status?.toLowerCase() !== "inactive" && {
                      id: record?.id,
