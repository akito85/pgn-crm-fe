...
import { createAssets, getDetailAssets, getListAnsi, getListAssetBrand, getListAssetName, getListAssetType, getListGsize, getListProductName, getListServicePointName, updateAsstes } from '../../../../../redux/slices/account_management/MasterData/assets_slice';
...
const FormAssets = ({ type }) => {
    ...
    const assert = useCallback((data) => {
        if (data) {
            form.setFieldsValue({
                serialNumber: data?.informationDto?.serialNumber?.replace(/\s+/g, ' ').trim() + ' ',
                ...
                productVersion: data?.informationDto?.productInformation?.value ? data?.informationDto?.productInformation?.value : null,
    ...
    useEffect(() => {
        if (location?.state?.id && type === 'update') {
            ...
            form.setFieldsValue({
                serviceType: data_service_type?.filter(item => item?.glbTypeValId === 608)[0]?.glbTypeValId
            })
        }
    }, [assert, data_detail, location?.state?.id, type, data_service_type, form]);

    // use memo ddl
    const assetName = useMemo(() => (data_asset_name?.map(item => ({ name: item?.text, value: item?.id, id: item?.id }))), [data_asset_name])
    const assetType = useMemo(() => (data_asset_type?.map(item => ({ name: item?.text, value: item?.id, id: item?.id }))), [data_asset_type]);
    const brandType = useMemo(() => (data_brand_type?.map(item => ({ name: item?.text, value: item?.id, id: item?.id }))), [data_brand_type]);
    const productName = useMemo(() => (data_product_name?.map(item => ({ name: item?.productName, value: item?.id, id: item?.id }))), [data_product_name]);
    const gSize = useMemo(() => (data_gsize?.map(item => ({ name: item?.text, value: item?.id, id: item?.id }))), [data_gsize]);
    const serviceTypeName = useMemo(() => (data_service_type?.map(item => ({ name: item?.name, value: item?.glbTypeValId, id: item?.id }))), [data_service_type]);
    const ansiName = useMemo(() => (data_ansi?.map(item => ({ name: item?.text, value: item?.id, id: item?.id }))), [data_ansi])
    ...
    // handle confirmation
    const handleFinish = useCallback(async (formValue) => {
        try {
            let body;
            let validateValueObj;
            const intParsedValue = {
                productVersion: hasValue(formValue?.productVersion) ? formValue?.productVersion : null,
                inletDiameter: hasValue(formValue?.inletDiameter) ? parseFloat(formValue?.inletDiameter) : null,
                outletDiameter: hasValue(formValue?.outletDiameter) ? parseFloat(formValue?.outletDiameter) : null,
                maximumInletPressure: hasValue(formValue?.maximumInletPressure) ? parseFloat(formValue?.maximumInletPressure) : null,
                maximumOutletPressure: hasValue(formValue?.maximumOutletPressure) ? parseFloat(formValue?.maximumOutletPressure) : null,
                minimumInletPressure: hasValue(formValue?.minimumInletPressure) ? parseFloat(formValue?.minimumInletPressure) : null,
                minimumOutletPressure: hasValue(formValue?.minimumOutletPressure) ? parseFloat(formValue?.minimumOutletPressure) : null,
                streamAmount: hasValue(formValue?.streamAmount) ? parseFloat(formValue?.streamAmount) : null,
                settingPressure: hasValue(formValue?.settingPressure) ? parseFloat(formValue?.settingPressure) : null,
                length: hasValue(formValue?.length) ? parseFloat(formValue?.length) : null,
                boltHoleAmount: hasValue(formValue?.boltHoleAmount) ? parseInt(formValue?.boltHoleAmount) : null,
                minimumCapacity: hasValue(formValue?.minimumCapacity) ? parseFloat(formValue?.minimumCapacity) : null,
                maximumCapacity: hasValue(formValue?.maximumCapacity) ? parseFloat(formValue?.maximumCapacity) : null,
                maxFlowCapacityPerStream: hasValue(formValue?.maxFlowCapacityPerStream) ? parseFloat(formValue?.maxFlowCapacityPerStream) : null,
                custodyTransfer: checkCustody,
                year: hasValue(formValue?.year) ? parseInt(moment(formValue?.year).format(dateFormatting.year_only)) : null,
            ...
            setGetAssetName(assetName?.filter(item => item?.value === formValue?.assetName)[0]?.name);
            setGetAssetType(assetType?.filter(item => item?.value === formValue?.type)[0]?.name);
            setGetBrandType(brandType?.filter(item => item?.value === formValue?.brand)[0]?.name);
            setGetProductName(productName?.filter(item => item?.value === formValue?.productVersion)[0]?.name);
            setGetGSize(gSize?.filter(item => item?.value === formValue?.gSize)[0]?.name);
            setGetServiceTypeName(serviceTypeName?.filter(item => item?.value === formValue?.serviceType)[0]?.name);
            setGetAnsiName(ansiName?.filter(item => item?.value === formValue?.ansi)[0]?.name);

        } catch (error) {

        }
    }, [ansiName, assetName, assetType, brandType, checkCustody, gSize, location?.state?.id, productName, serviceTypeName, type, dispatch]);
                ...
                <Form form={form} layout={'vertical'} onFinish={handleFinish}>
                    <BaseContainer header={'asset information'}>
                        <div className='w-full grid grid-cols-3 gap-5'>
                            ...
                            >
                                <SelectComponent disabled={type === 'update'} onChange={handleAssetName}>
                                    {assetName?.map((key, item) =>
                                    (
                                        <Select.Option key={key} value={key?.value}>{key?.name}</Select.Option>
                                    ))}
                                </SelectComponent>
                            </Form.Item>
                        </div>
                        ...
                        <div className='w-full grid grid-cols-3 gap-5'>
                            ...
                            >
                                <DatePicker picker='year' className='w-full' disabled={type === 'update'} />
                            </Form.Item>
                            <Form.Item
            ...
            >
                <div>
                    ...
                    <div className='w-full grid grid-cols-3'>
                        ...
                        <DetailText label={'Serial Number'}>{body?.body?.serialNumber?.replace(/\s+/g, ' ').trim() + ' '}</DetailText>
                        <DetailText label={'Brand'}>{getBrandType}</DetailText>
                    </div>
                    <div className='w-full grid grid-cols-3'>
                        <DetailText label={'Year'}>{body?.body?.year}</DetailText>
                        <DetailText label={'Custody Transfer'}>{body?.body?.custodyTransfer === true ? 'Yes' : 'No'}</DetailText>
                    </div>
                    <div className="text-primary text-xs font-bold uppercase py-4">
                        asset attribute
                    </div>
                    <div className='w-full grid grid-cols-3'>
                        <DetailText label={'Inlet Diameter'}>{body?.body?.inletDiameter}</DetailText>
                        <DetailText label={'Outlet Diameter'}>{body?.body?.outletDiameter}</DetailText>
                        <DetailText label={'Minimum Inlet Pressure'}>{body?.body?.minimumInletPressure}</DetailText>
                    </div>
                    <div className='w-full grid grid-cols-3'>
                        <DetailText label={'Maximum Inlet Pressure'}>{body?.body?.maximumInletPressure}</DetailText>
                        <DetailText label={'Minimum Outlet Pressure'}>{body?.body?.minimumOutletPressure}</DetailText>
                        <DetailText label={'Maximum Outlet Pressure'}>{body?.body?.maximumOutletPressure}</DetailText>
                    </div>
                    <div className='w-full grid grid-cols-3'>
                        <DetailText label={'Max Flow Capacity per Stream'}>{body?.body?.maxFlowCapacityPerStream}</DetailText>
                        <DetailText label={'Stream Amount'}>{body?.body?.streamAmount}</DetailText>
                        <DetailText label={'G Size'}>{getGSize}</DetailText>
                    </div>
                    <div className='w-full grid grid-cols-3'>
                        <DetailText label={'Setting Pressure'}>{body?.body?.settingPressure}</DetailText>
                        <DetailText label={'Length'}>{body?.body?.length}</DetailText>
                        <DetailText label={'Bolt Hole Amount'}>{body?.body?.boltHoleAmount}</DetailText>
                    </div>
                    <div className='w-full grid grid-cols-3'>
                        <DetailText label={'Minimum Capacity'}>{body?.body?.minimumCapacity}</DetailText>
                        <DetailText label={'Maximum Capacity'}>{body?.body?.maximumCapacity}</DetailText>
