  ...
  const columnHistoryAssignment = (
    ...
    return [
      ...
      },
      {
        ...
        align: "left",
        // sorter: true,
        filteredValue: search?.["servicePointName"]
          ? [search?.["servicePointName"]]
          : null,
        ...
        <BaseContainer header={"asset information"}>
          <div className="w-full grid grid-cols-3">
            <DetailText label={"Product Name"}>
              {data_detail?.informationDto?.productInformation?.name}
            </DetailText>
            <DetailText label={"Service Type"}>
              {data_detail?.informationDto?.serviceType}
            </DetailText>
            <DetailText label={"Asset Name"}>
              {data_detail?.informationDto?.assetName}
            </DetailText>
            <DetailText label={"Asset Type"}>
              {data_detail?.informationDto?.assetType}
            </DetailText>
            <DetailText label={"Serial Number"}>
              {data_detail?.informationDto?.serialNumber}
            </DetailText>
            <DetailText label={"Brand"}>
              {data_detail?.informationDto?.brand}
            </DetailText>
            <DetailText label={"Year"}>
              {data_detail?.informationDto?.year}
            </DetailText>
            <DetailText label={"Custody Transfer"}>
              {data_detail?.informationDto?.custodyTransfer === true
                ? "Yes"
                : "No"}
            </DetailText>
          </div>
          ...
          <div className="w-full grid grid-cols-3">
            <DetailText label={"Inlet Diameter"}>
              {data_detail?.attribute?.inletDiameter}
            </DetailText>
            <DetailText label={"Outlet Diameter"}>
              {data_detail?.attribute?.outletDiameter}
            </DetailText>
            <DetailText label={"Minimum Inlet Pressure"}>
              {data_detail?.attribute?.minInletPressure}
            </DetailText>
          </div>
          <div className="w-full grid grid-cols-3">
            <DetailText label={"Maximum Inlet Pressure"}>
              {data_detail?.attribute?.maxInletPressure}
            </DetailText>
            <DetailText label={"Minimum Outlet Pressure"}>
              {data_detail?.attribute?.minimumOutletPressure}
            </DetailText>
            <DetailText label={"Maximum Outlet Pressure"}>
              {data_detail?.attribute?.maxOutletPressure}
            </DetailText>
          </div>
          <div className="w-full grid grid-cols-3">
            <DetailText label={"Max Flow Capacity Per Stream"}>
              {data_detail?.attribute?.maxFlowCapacityPerStream}
            </DetailText>
            <DetailText label={"Stream Amount"}>
              {data_detail?.attribute?.streamAmount}
            </DetailText>
            <DetailText label={"G Size"}>
              {data_detail?.attribute?.gsize}
            </DetailText>
          </div>
          <div className="w-full grid grid-cols-3">
            <DetailText label={"Setting Pressure"}>
              {data_detail?.attribute?.settingPressure}
            </DetailText>
            <DetailText label={"Length"}>
              {data_detail?.attribute?.length}
            </DetailText>
            <DetailText label={"Bolt Hole Amount"}>
              {data_detail?.attribute?.boltHoleAmount}
            </DetailText>
          </div>
          <div className="w-full grid grid-cols-3">
            <DetailText label={"Minimum Capacity"}>
              {data_detail?.attribute?.minCapacity}
            </DetailText>
            <DetailText label={"Maximum Capacity"}>
              {data_detail?.attribute?.maxCapacity}
            </DetailText>
            <DetailText label={"Class/ANSI"}>
              {data_detail?.attribute?.ansi}
            </DetailText>
          ...
          <div className="w-full grid grid-cols-1">
            <DetailText label={"Description"}>
              {data_detail?.attribute?.description}
            </DetailText>
          </div>
          <div className="py-4">
            <div className="text-primary text-xs font-bold uppercase">
          ...
          <div className="w-full grid ">
            <DetailText label={"Status"}>
              {toTitleCase(data_detail?.assetStatusDto?.status)}
            </DetailText>
          </div>
          <div className="w-full grid ">
            <DetailText label={"Asset Location"}>
              {data_detail?.assetStatusDto?.assetLocation}
            </DetailText>
          </div>
        </BaseContainer>
        <BaseContainer header={"history log information"}>
          <div className="w-full grid grid-cols-5">
            ...
            <DetailText label={"Created Date"}>
              {hasValue(data_detail?.history?.createdDate) &&
                moment(data_detail?.history?.createdDate).format(
                  dateFormatting?.dateTime,
                )}
            </DetailText>
            <DetailText label={"Created By"}>
              {data_detail?.history?.createdBy}
            </DetailText>
            <DetailText label={"Updated Date"}>
              {hasValue(data_detail?.history?.updatedDate) &&
                moment(data_detail?.history?.updatedDate).format(
                  dateFormatting?.dateTime,
                )}
            </DetailText>
            <DetailText label={"Updated By"}>
              {data_detail?.history?.updatedBy}
            </DetailText>
          </div>
        </BaseContainer>
