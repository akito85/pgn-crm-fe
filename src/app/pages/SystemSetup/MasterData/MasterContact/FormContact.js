...
import { createContact, getDetailContact, getInputType, getListJob, getListPosition, resetDataDetail, updateContact, getContactType, getCountryCode, getCountryZone } from '../../../../../redux/slices/account_management/MasterData/contact_slice';
...
const FormContact = ({ type }) => {
    const {
        ...
        data_country_code,
        data_country_zone,
        temp_country_zone } = useSelector(state => state.contact);
    ...
    const dispatchCountryZone = useCallback((e) => {
    ...
    const assert = useCallback((data) => {
        if (data) {
            ...
            setDataTable(data?.contactDetail?.map((item, index) => {
            ...
            if (dataTable?.length === 0) {
                ...
                if (type === 'create') {
                    ...
                    bodyByTypeForm = {
                        ...
                        viewDetails: dataTable?.map((item, index) => {
                            return {
                                contactDetailId: hasValue(item?.contactDetailId) ? item?.contactDetailId : null,
                                type: item?.type?.value,
                                inputType: item?.inputType?.value,
                                value: hasValue(item?.values[0]?.value) ? item?.values[0]?.value : null,
                                prefix1: hasValue(item?.values[0]?.prefix_1?.value) ? item?.values[0]?.prefix_1?.value : null,
    ...
    const handleConfirm = () => {
        if (bodyError?.action === "UPDATE_CONTACT" || bodyError?.action === "CREATE_CONTACT") {
            handleSave();
        } else {
            dispatch(getDetailContact(location?.state?.id));
        }
    ...
    const handleDispatcher = useCallback((e) => {
        if (inputTypeRow) {
            return {
    ...
    const editRecords = useCallback((record) => {
        setSelectedType(record?.type?.value)
        if (record?.inputType?.value === 748 || record?.inputType?.value === 751) {
            if (hasValue(record?.prefix_1?.value) && record?.hasOwnProperty('prefix_2')) {
    ...
    const columnContact = useMemo(() => {
        return [
            ...
            },
            {
                ...
                render: (text) => renderColumn('type', searchedColumnInlane, searchTextInlane, text?.label, false, 'input', search)
            },
            {
                title: "INPUT TYPE",
                dataIndex: "inputType",
                ...
                render: (text) => renderColumn('inputType', searchedColumnInlane, searchTextInlane, text?.label, false, 'input', search)
            },
            {
                title: "VALUE",
                dataIndex: "value",
                ...
                render: (text, record, id) => {
                    return renderColumn('value', searchedColumnInlane, searchTextInlane, record?.fullValue, true, 'input', search)
                }
            },
        ]
            ...
            >
                <div>
                    ...
                    <div className='w-full grid grid-cols-3'>
                        <DetailText label={'First Name'}>{body?.body?.firstName}</DetailText>
                        <DetailText label={'Middle Name'}>{body?.body?.middleName}</DetailText>
                        <DetailText label={'Last Name'}>{body?.body?.lastName}</DetailText>
                    </div>
                    <div className='w-full grid grid-cols-3'>
                        <DetailText label={'Job'}>{data_job?.filter(item => item?.value === body?.body?.jobId)[0]?.name}</DetailText>
                        <DetailText label={'Position'}>{data_position?.filter(item => item?.value === body?.body?.positionId)[0]?.name}</DetailText>
                    </div>
                    <div className="text-primary text-xs font-bold uppercase py-4">
                        Contact Detail
                    </div>
