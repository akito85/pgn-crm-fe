...
import { createLocation, getDetailLocation, getLocationPaginate, getLocationParentType, getLocationReference, getLocationType, setClearLocationParent, updateLocation } from '../../../../../redux/slices/account_management/MasterData/location_slice';
...
const FormLocations = ({ type }) => {
    const { loading, data_location_reference, data_location_type, data_detail, data } = useSelector(state => state.location);
    ...
    const dataLocation = data_location_type?.filter(item => (item?.id !== 2346))?.map(item => ({ value: item?.id, name: item?.name }));
    ...
    useEffect(() => {
        if (form.getFieldsValue()?.locationType === 103) {
            ...
            dispatch(getLocationPaginate({ typeId: form.getFieldsValue()?.locationType, search: reqSearch, sort, page, pageSize }))
        }
        // else if (type === 'update' || hasValue(form.getFieldsValue()?.locationParentType) || hasValue(form?.getFieldsValue().locationParent)) {
            ...
            const reqSearch = encodeURIComponent(JSON.stringify(search))
            dispatch(getLocationPaginate({ typeId: form.getFieldsValue()?.locationParentType, search: reqSearch, sort, page, pageSize }))
        } else {
            dispatch(setClearLocationParent())
        }
    }, [selectedParentType, form, dispatch, page, pageSize, type, modalChoose, search, page, pageSize, sort])
    ...
    const handleSave = async () => {
        handleCancel();
        if (type === 'update') {
            await dispatch(updateLocation({ body: body?.body, id: location?.state?.id }))?.unwrap()
        } else {
    ...
    const columns = (
        ...
        return arrayCols?.filter(item => (item?.dataIndex === 'no' || item.dataIndex === 'locationName' || item.dataIndex === 'locationType' || item?.dataIndex === 'locationParent' || item.dataIndex === 'locationReference' || item?.dataIndex === 'locationCode' || item?.dataIndex === 'status' || item?.dataIndex === 'action'));
    }
                    ...
                    <BaseContainer header={'location information'}>
                        ...
                        <div className='w-full grid grid-cols-3 gap-5'>
                            <Form.Item
                                ...
                                rules={(form?.getFieldsValue()?.locationType !== 103 && form?.getFieldsValue()?.locationType !== undefined) && formMessageRequired('Location Parent Type')}
                            >
                            ...
                            <Form.Item
                                ...
                                name={'locationParent'}
                                rules={(form?.getFieldsValue()?.locationType !== 103 && form?.getFieldsValue()?.locationType !== undefined) && formMessageRequired('Location Parent')}
                            >
                                <div className="flex flex-row">
                                    <Input.Group compact>
                                        ...
                                        />
                                        <Button
                                            disabled={form?.getFieldsValue()?.locationType === 103 || hasValue(form?.getFieldsValue()?.locationType) === false}
                                            type="primary"
                                            onClick={() => {
                                                // setKeyModal(1);
                                                setModalChoose(true);
                ...
                <div>
                    ...
                    <div className='w-full grid grid-cols-3'>
                        <DetailText label={'Location Type'}>{
                            dataLocation?.filter(item => (item?.value === body?.body?.locationType))[0]?.name
                        }</DetailText>
                    ...
                    <div className='w-full grid grid-cols-3'>
                        <DetailText label={'Location Parent Type'}>{
                            dataLocation?.filter(item => (item?.value === body?.body?.locationParentType))[0]?.name
                        }</DetailText>
                        <DetailText label={'Location Parent'}>{
                            chooseLocationParent?.name
                        }</DetailText>
                        <DetailText label={'Location Reference'}>{
                            data_location_reference?.filter(item => (item?.value === body?.body?.locationReference))[0]?.name
                        }</DetailText>
                    </div>
                </div>
            </ModalCustom>
