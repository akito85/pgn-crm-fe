...
const EditableCell = ({
  ...
  const resetValueDate = (current, dataIndex) => {
    if (
      dataIndex &&
      moment(current).isAfter(moment(form.getFieldValue("endDate")), "day")
    ) {
...
const TableInlinegasSource = ({
  ...
  const save = async (key) => {
    try {
      ...
      const hasDuplicateRow = filteredTable?.filter(
        (item) => item?.costCenterId === updatedRow?.costCenterId,
      );
      ...
      <ButtonComponent
        ...
        icon={
          <SVGIcon
            name="IconDelete"
            width={24}
            color={editingKey ? "#C0BEC6" : "#D90000"}
          />
