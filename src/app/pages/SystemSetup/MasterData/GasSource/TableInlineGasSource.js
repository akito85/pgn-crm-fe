import { Checkbox, DatePicker, Form, Input, InputNumber, Pagination, Popover, Select, Space, Table, Tooltip } from "antd";
...
const EditableCell = ({
    ...
    const resetValueDate = (current, dataIndex) => {
        if (dataIndex && moment(current).isAfter(moment(form.getFieldValue("endDate")), 'day')) {
    ...
    const getInputNode = (inputType, options) => {
        switch (inputType) {
            case "text":
                return <Input onInput={onInput} maxLength={maxLength} />;
            case "input_regex":
                return (
                    <Input
                        onInput={onInput}
                    ...
                    <DatePicker
                        ...
                        onChange={(date) => {
                            if (dataIndex === 'startDate') {
                                resetValueDate(date, dataIndex)
                            }
                        }}
                    />
                );
            case "input_password":
                return <Input type={showPassword[key] === true ? "password" : "text"} onInput={onInput} maxLength={maxLength} />;
            case "description":
...
// component table inline
const TableInlinegasSource = ({
    onDataChange,
    cols,
    tableData,
    mode,
    ...
    const save = async (key) => {
        try {
            ...
            const filteredTable = tableData?.filter(item => item?.key !== key);
            const item = newData[index];
            const updatedRow = { ...item, ...row };
            const hasDuplicateRow = filteredTable?.filter(item => item?.costCenterId === updatedRow?.costCenterId);
            // validate duplicate cost center
            if (hasDuplicateRow?.length > 0) {
                setDoubleCostCenter(true);
            } else {
    ...
    const renderDelete = (record) => {
        ...
        return record.id ? (
            ...
            <ButtonComponent
                ...
                disabled={editingKey !== ""}
                icon={
                    <SVGIcon name="IconDelete" width={24} color={editingKey ? "#C0BEC6" : "#D90000"}/>
    ...
    const renderUpdate = (record) => {
        ...
        return record.status === "INACTIVE" ? (
            ...
            <ButtonComponent
                onClick={() => edit(record)}
                disabled={editingKey !== ""}
                icon={<SVGIcon name="IconEdit" width={24} color={editingKey ? "#8D91A0" : "#ACC424"}/>}
                border={false}
            />
        );
    };
                    ...
                    <Space className="my-2 gap-2">
                        {useDynamicAction ? (
                            ...
                            <>
                                ...
                                {record.id ? (
                                    ...
                                    <ButtonComponent
                                        onClick={() => deleteRow(record.key)}
                                        disabled={editingKey !== ""}
                                        icon={<SVGIcon name="IconDelete" width={24} color={!editingKey ? "#D90000" : "#8D91A0"}/>}
                                        border={false}
                                    />
                                )}
                            </>
                            ...
                            <>
                                ...
                                {actionButton?.includes("update") && renderUpdate(record)
                                    /* (
                                        <ButtonComponent
                                            onClick={() => edit(record)}
                                            disabled={editingKey !== ""}
...
export default TableInlinegasSource;