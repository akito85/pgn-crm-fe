...
import { getCollectionActivityList } from "../../../../../../redux/slices/system_setup/master_data/collectionTemplate";
...
const EditableCell = ({
  ...
  const rules = required
    ? [
        {
          required: true,
          message: `Please ${inputType === "select" || inputType === "selectYN" ? "select" : "input"} ${title.toLowerCase()}!`,
        },
      ]
    : undefined;
  ...
  const getInputNode = () => {
    if (readOnly) {
      return <InputComponent disabled style={{ color: "rgba(0,0,0,0.85)" }} />;
    }
    switch (inputType) {
      case "select":
        return (
          <Select
            ...
            onChange={
              dataIndex === "activityName" ? handleActivitySelect : undefined
            }
          >
            {(options || []).map((opt) => (
...
const FunctionalActivitiesCollectionTemplate = ({
  type,
  data = [],
  updateData = () => {},
  storedData = false,
  ...
  const { data_activities } = useSelector((state) => state.collectionTemplate);
  ...
  const activityOptions = (data_activities || []).map((item) => ({
    value: item.collectionActivityId,
  ...
  const columns = [
    ...
    },
    {
      ...
      required: true,
      render: (_, record) => {
        const opt = activityOptions.find(
          (o) => o.value === record.activityName,
        );
        return opt?.label || record.activityName || "-";
      },
    },
    {
  ...
  const mergedColumns = columns
    ...
    .map((col) => ({
      ...
      onCell: (record) => ({
        ...
        options: col.dataIndex === "activityName" ? activityOptions : undefined,
...
export default FunctionalActivitiesCollectionTemplate;
