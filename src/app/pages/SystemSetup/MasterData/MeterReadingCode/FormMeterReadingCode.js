...
import {
  createMeterReadingCode,
  getDetailMeterReadingCode,
  getListCostCenterMeterReading,
  updateMeterReadingCode,
} from "../../../../../redux/slices/account_management/MasterData/meter_reading_code_slice";
...
const FormMeterReadingCode = ({ type }) => {
  const { data_cost_center, loading, data_detail } = useSelector(
          ...
          <BaseContainer header={"meter reading codes information"}>
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <SelectComponent
                  disabled={
                    tokenParsed?.userLevel !== "Super User" &&
                    tokenParsed?.userLevel !== "Admin Entity"
                  }
                >
        ...
        <div>
          ...
          <div className="w-full grid grid-cols-2">
            <DetailText label={"Cost Center"}>
              {
                data_cost_center?.filter(
                  (item) => item?.value === body?.body?.costCenterId,
                )[0]?.name
              }
            </DetailText>
