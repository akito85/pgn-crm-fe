...
import {
  getAllActivityTypePaginate,deleteActivityType
...
import { columns } from "./ColumnActivityTypeView";
...
const ViewActivityType = () => {
  ...
  const handleConfirmModalDelete = () => {
    ...
    dispatch(deleteActivityType({ id: idSelected }))
    .unwrap()
    .then((data) => {
      dispatch(
        getAllActivityTypePaginate({
          page,
          pageSize,
  ...
  const itemsActionView = () => [
    {
      ...
      render: (
        <NavLink
          to={DEBT_AND_COLLECTION_ROUTES.CREATE_ACTIVITY_TYPE}
      ...
      render: (record, data_length) => {
        return (
          <Tooltip title="Detail">
            <Link
              to={DEBT_AND_COLLECTION_ROUTES.DETAIL_ACTIVITY_TYPE}
              state={{ id: record?.activityTypeId }}
            >
        ...
        render: (record, data) => {
          return (
            <Tooltip title="Update">
              <Link
                to={DEBT_AND_COLLECTION_ROUTES.UPDATE_ACTIVITY_TYPE}
                state={{ id: record?.activityTypeId }}
              >
                <SVGIcon
                  name="IconEdit"
            ...
            <Tooltip title="Delete">
              <div className="pt-1">
                <SVGIcon
                  name="IconDelete"
                  width={24}
                  onClick={() => handleOpenDelete(record?.activityTypeId)}
                />
              </div>
            </Tooltip>
          )
  ...
  useEffect(() => {
    if (dataActivityType) {
      let result = dataActivityType?.result || [];
      const totalData = dataActivityType?.page?.totalElements || 0;
      setDataTable(result);
    ...
    <LayoutMenu>
      ...
      >
        ...
        <div className="flex flex-col w-full">
          ...
          />
          <BaseContainer header={"Activity Type List"}>
            <TablePaginationNew
