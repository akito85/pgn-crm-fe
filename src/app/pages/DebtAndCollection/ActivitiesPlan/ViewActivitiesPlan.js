...
import {
  ...
  getAllActivityPlanDetailByAccountNumePaginate,
  deleteActivity,
  downloadEvidence,
  createBulkAcitivityPlan,
  validateBulkActivityPlan,
  downloadTemplateBulkActivityPlan
...
import { columnmActivitiesPlan } from "./ColumnActivitiesPlan";
...
const routes = [
  ...
  },
  {
    ...
    breadcrumbName: "Activities Plan",
  },
];
...
const ViewActivityAction = (props) => {
  ...
  const [tabHeader, setTabHeader] = useState("Activity Plan Information");
  ...
  const itemsActionView = () => [
    {
      ...
      render: (
        <NavLink
          to={DEBT_AND_COLLECTION_ROUTES.CREATE_ACTIVITIES_PLAN}
      ...
      render: (record, data_length) => {
        return (
          <Tooltip title="Detail">
            <Link
              to={DEBT_AND_COLLECTION_ROUTES.DETAIL_ACTIVITIES_PLAN}
              state={{ id: record?.id }}
            >
        ...
        render: (record, data) => {
          return (
            <Tooltip title="Update">
              <Link
                to={DEBT_AND_COLLECTION_ROUTES.UPDATE_ACTIVITIES_PLAN}
  ...
  // data tabs
  const tabs = [
    { value: "Activity Plan Information" },
  ...
  const handleDownloadTemplate = ()=> {
    dispatch(downloadTemplateBulkActivityPlan())
  }
  ...
  const saveAction = async () => {
    try {
      ...
      await dispatch(createBulkAcitivityPlan(formData)).unwrap()
      .then(() => {
        form.resetFields();
        setTabHeader("Activity Plan Information"); 
        dispatch(
          getAllActivityPlanDetailByAccountNumePaginate({
            accountNum: selectedAccountNum,
            page: pageActivities,
  ...
  const onFinish = async (formValue) => {
    ...
    try {
      ...
      dispatch(validateBulkActivityPlan(formData))
        .unwrap()
        .then(async (data) => {
          ...
          <BaseContainer header={"Account"}>
            ...
            {selectedAccountNum && (
              <div className="mt-6 border rounded-lg p-4 bg-white shadow-sm">
                ...
                {/* Tab Content */}
                {tabHeader === "Activity Plan Information" && (
