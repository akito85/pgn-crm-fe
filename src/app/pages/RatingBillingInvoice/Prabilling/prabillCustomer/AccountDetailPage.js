...
const TAB_CONFIGS = [
  { key: "0", label: "Service Agreement", dataKey: "saData", scrollX: 2500, action: null },
  ...
  { key: "3", label: "SA Price Rule", dataKey: "saPrcRuleDetData", scrollX: 1100, action: "getCustomerSaPrcRuleDetData" },
...
const AccountDetailPage = () => {
  ...
  const [fixedColumnsSA, setFixedColumnsSA] = useState(() => createFixedColumnsState(["no"]));
  ...
  const processedColumns = {
    sa: useMemo(() => applyFixedColumns(saColumns, fixedColumnsSA), [saColumns, fixedColumnsSA]),
    ...
    saPrcRuleDet: useMemo(() => applyFixedColumns(saPrcRuleDetColumns, fixedColumnsPrice), [saPrcRuleDetColumns, fixedColumnsPrice]),
  ...
  // Column definitions
  const columnDefs = {
    sa: useMemo(() => saColumns.map((col) => ({ key: col.key, title: col.title })), [saColumns]),
      ...
      {renderInfoCard("INIT / CUSTOMER & ACCOUNT INFORMATION", (
        <>
          <div className="mb-4">
            ...
            {renderDetailGrid([
              { label: "Init Code", value: renderValue(firstHeaderData.initCode) },
              { label: "Billing Cycle", value: renderValue(firstHeaderData.billingCycle) },
              { label: "Bill Period", value: renderValue(firstHeaderData.billPeriod) },
              { label: "Customer Number", value: renderValue(firstHeaderData.customerNumber) },
              { label: "Customer Name", value: renderValue(firstHeaderData.customerName) },
              { label: "Customer Type", value: renderValue(firstHeaderData.customerType) },
            ])}
          </div>
          ...
          <div>
            ...
            {renderDetailGrid([
              { label: "Account Number", value: renderValue(firstHeaderData.accountNumber) },
              { label: "Account Name", value: renderValue(firstHeaderData.accountName) },
              { label: "Account Status", value: firstHeaderData.accountStatus ? (
                <Tag color={firstHeaderData.accountStatus === "ACTIVE" ? "green" : "red"}>{firstHeaderData.accountStatus}</Tag>
              ) : "" },
              { label: "Account Group", value: renderValue(firstHeaderData.accountGroup) },
              { label: "SOR", value: renderValue(firstHeaderData.sor) },
              { label: "Cost Center", value: renderValue(firstHeaderData.costCenter) },
              { label: "Meter Reading Code", value: renderValue(firstHeaderData.meterReadingCode) },
              { label: "Account Segment", value: renderValue(firstHeaderData.accountSegment) },
              { label: "Account Group Type", value: renderValue(firstHeaderData.accountGroupType) },
              { label: "Account Type", value: renderValue(firstHeaderData.accountType) },
            ])}
          </div>
        </>
      ))}
            ...
            return (
              ...
              >
                <TableRBI
                  ...
                  pageSize={tab.key === "0" ? tabData.data.length : currentPag.pageSize}
