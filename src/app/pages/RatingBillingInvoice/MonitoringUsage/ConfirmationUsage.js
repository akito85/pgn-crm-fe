...
const ConfirmationUsage = ({
  ...
  const [tabPages] = useState([{ value: "Upload" }, { value: "Approval" }]);
  ...
  const handleConfirm = async () => {
    ...
    const resultAction = await dispatcher(saveSubmitData(payload));

    if (saveSubmitData.fulfilled.match(resultAction)) {
      onSaveSuccess();
      handleCancel();
          ...
          <>
            ...
            <div className={"w-full grid grid-cols-4"}>
              <DetailText label={"Batch ID"}>
                {data_detail?.batchInformation?.batchId}
              </DetailText>
              <DetailText label={"Upload Type"}>
                {data_detail?.batchInformation?.uploadType}
              </DetailText>
              <DetailText label={"Upload Date"}>
                {data_detail?.batchInformation?.uploadDate}
              </DetailText>
              <DetailText label={"Upload By"}>
                {data_detail?.batchInformation?.uploadBy}
              </DetailText>
            </div>
            <div className={"w-full grid grid-cols-4"}>
              <DetailText label={"Total Data"}>
                {data_detail?.batchInformation?.totalUsage}
              </DetailText>
              <DetailText label={"Total Succeed"}>
                {data_detail?.batchInformation?.totalSucceed}
              </DetailText>
              <DetailText label={"Total Progress"}>
                {data_detail?.batchInformation?.totalProgress}
              </DetailText>
              <DetailText label={"Total Failed"}>
                {data_detail?.batchInformation?.totalFailed}
              </DetailText>
            </div>
            <div className={"w-full grid grid-cols-4"}>
              <DetailText label={"Status"}>
                {toTitleCase(data_detail?.batchInformation?.status)}
              </DetailText>
            </div>
            <div className="my-10">
              <TableRBI
