        ...
        <div className="w-full p-5">
          ...
          <div className="w-full grid grid-cols-3 gap-4">
            ...
            <DetailText label={"Customer Identification Number"}>
              {dataCI?.customerIdentificationNumber}
            </DetailText>
          </div>
              ...
              <>
                <DetailText label={"First Name"}>
                  {!dataCheck
                    ...
                    : dataCustomer?.customerInformation?.firstName}
                </DetailText>
                <DetailText label={"Middle Name"}>
                  {!dataCheck
                    ? (dataCI?.middleName || "").toUpperCase()
                    : dataCustomer?.customerInformation?.middleName}
                </DetailText>
                <DetailText label={"Last Name"}>
                  {!dataCheck
                    ? (dataCI?.lastName || "").toUpperCase()
                    : dataCustomer?.customerInformation?.lastName}
                </DetailText>
              </>
            )}

            {dataCI?.customerType === 58 ? (
              <DetailText label={"Customer Name"}>
                {!dataCheck
                  ? (dataCI?.customerName || "").toUpperCase()
                  : dataCustomer?.customerInformation?.customerName}
              </DetailText>
            ) : (
              <DetailText label={"Customer Name"}>
                {!dataCheck
                  ? `${(dataCI?.firstName || "").toUpperCase()} ${(
                      dataCI?.middleName || ""
                    ).toUpperCase()} ${(dataCI?.lastName || "").toUpperCase()}`
                  : dataCustomer?.customerInformation?.customerName}
              </DetailText>
            )}
