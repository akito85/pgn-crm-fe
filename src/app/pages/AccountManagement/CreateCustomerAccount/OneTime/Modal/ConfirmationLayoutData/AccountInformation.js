...
const AccountInformation = ({
  ...
  const labelIndustrialSector = data_industrialSector?.reduce((result, item) => {
    if (item.children && item.children.find(child => child.value === dataAI?.industrialSector)) {
      result = item.children.find(child => child.value === dataAI?.industrialSector).title;
    }
    return result;
  }, '')
