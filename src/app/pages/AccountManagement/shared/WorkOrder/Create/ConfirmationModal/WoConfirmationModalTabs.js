...
const attachmentColumns = [
  ...
  { title: "SIZE", dataIndex: "fileSize", width: 120, render: (v) => typeof v === "number" ? bytesConverter(v) : (v || "-") },
  {
    title: "ACTION", align: "center", width: 80, fixed: "right",
    render: (_, record) => record.base64 ? (
      <Tooltip title="Preview">
        <Button type="table-action" onClick={() => previewFileAttachment(record.base64)}>
...
const WoConfirmationModalTabs = ({ formValues, activityData, dataRequirements, approvalData, attachments, activeTabKey }) => {
  ...
  const items = [
    ...
    },
    {
      ...
      children: (
        <div className="flex flex-col gap-4">
          <NxBaseContainer border header="WO Attachments">
            <NxTable
              idTable="confirm-attachment-wo-table"
          ...
          <NxBaseContainer border header="Activity Attachments">
            <NxTable
              idTable="confirm-attachment-activity-table"
              dataSource={activityAttachments.map((item, i) => ({ ...item, key: item.key || i }))}
              columns={[
                { title: "NO", width: 60, align: "center", render: (_, __, i) => i + 1 },
                { title: "ACTIVITY", dataIndex: "activityName", width: 160, render: (v) => v || "-" },
                { title: "FILE NAME", dataIndex: "fileName", render: (v) => v || "-" },
                { title: "SIZE", dataIndex: "fileSize", width: 120, render: (v) => typeof v === "number" ? bytesConverter(v) : (v || "-") },
                {
                  title: "ACTION", align: "center", width: 80, fixed: "right",
  ...
  return <Tabs items={items} activeKey={activeTabKey} onChange={() => {}} />;
};
