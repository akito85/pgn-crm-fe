...
import { getListCustomerContact } from "../../../../../../redux/slices/account_management/Customer/customerAccount";
...
const CustomerContactList = ({ id = 0, dispatch = () => {} }) => {
  ...
  useEffect(() => {
    if (
      ...
      data_customerDetailContact.result
      // data_customerDetailContact.result.length > 0
    ) {
      setTotalElement(data_customerDetailContact?.page?.totalElements);
      const data = data_customerDetailContact?.result?.map(
        (contact, indexContact) => ({
          ...contact,
          key: indexContact + 1,
          contactDetail: (contact?.contactDetail || []).map(
            (detail, index) => ({
              ...detail,
              key: index + 1,
  ...
  return (
    <Fragment>
      <BaseContainer header={"CONTACT DETAIL LIST"}>
