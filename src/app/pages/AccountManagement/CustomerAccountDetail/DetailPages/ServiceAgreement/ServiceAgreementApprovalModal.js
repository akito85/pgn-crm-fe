...
const ServiceAgreementApprovalModal = ({
    ...
    // Selector
    const { list_saApproval, pagination_saApproval, loading } = useSelector(
    ...
    const dataSource = list_saApproval || [];
    ...
    const handleLoadMore = async () => {
        ...
        const totalPages = pagination_saApproval?.totalPages || 0;
    ...
    const hasMore =
        dataSource.length < (pagination_saApproval?.totalElements || 0);
    ...
    const handleSave = (formValue) => {
        const body = selectedRows
            .filter((row) => row.approvalType === "SERVICE_AGREEMENT")
            .map((row) => ({
                id: row.id,
        ...
        const inactiveBody = selectedRows
            .filter((row) => row.approvalType === "INACTIVE_SERVICE_AGREEMENT")
            .map((row) => ({
                id: row.id,
                approvalId: row.tappId,
                action,
                        ...
                        <div className="w-full grid grid-cols-1 gap-x-4 pt-[30px]">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <p className="text-sm font-semibold text-blue-600">
                                    {selectedRows.length}{" "}
                                    {selectedRows.length === 1 ? "row" : "rows"} will be{" "}
                                    {action === "APPROVE"
                                        ? "approved"
                                        : action === "REJECT"
                                            ? "rejected"
...
export default ServiceAgreementApprovalModal;
