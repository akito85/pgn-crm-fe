...
const UpdateServiceAgreement = ({ saType }) => {
        ...
        .then((data) => {
          if (data) {
            ...
            const mergeTextAdjustment =
              `${priceAdjustmentOne?.adjustmentText || ""} - ${priceAdjustmentTwo?.adjustmentText || ""}`.trim();
            ...
            data?.saInfo?.appHierId &&
              dispatch(getDetailApproval(data?.appHierId));
            data?.saInfo?.appHierId &&
            ...
            setSaInfoObj({
              ...
              alreadyGasIn:
                data?.saInfo?.alreadyGasIn === null
                  ? false
                  : data?.saInfo?.alreadyGasIn,
              serviceAgreementDate:
                data?.saInfo?.saDate !== null
                  ? moment(data?.saInfo?.saDate).clone()
                  : null,
  ...
  const handleSubmitForm = (formValue) => {
    ...
    const bodyIsDraft = {
      ...
      saDetail: {
        ...
        productPricing: {
          ...
          priceRuleTiering: dataPricing.map((item) => {
            return {
              ...
              max: item.max,
              isUnlimited:
                item.unlimited || item.unlimited == "Y" ? true : false,
                    ...
                    return {
                      attribute:
                        itemDetail.attribute !== undefined
                          ? parseInt(itemDetail.attributeId)
                          : null,
                      unit:
                        itemDetail.unit !== undefined ? itemDetail.unit : null,
                      value:
        ...
        taxImplicationPPN:
          dataSendTaxImplication?.taxImplicationPPN?.taxImplicationId || null,
        taxImplicationPPh:
          dataSendTaxImplication?.taxImplicationPPH?.taxImplicationId || null,
      },
  ...
  const handleConfirm = () => {
    ...
    dispatch(
      updateServiceAgreement({
        body: saRecordData.status === "ACTIVE" ? bodyIsActive : body,
      }),
    )
      .unwrap()
      .then(async (data) => {
