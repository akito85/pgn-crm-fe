...
const TablePricing = ({
	...
	useEffect(() => {
		if (PDselect !== 0) {
			...
			const findAdjustmentDetail = ddlPriceCode
				?.filter((a) => a.id === PDselect)?.[0]
			...
			formDetail.setFieldsValue({
				priceDetail: findPriceDetail !== undefined ? findPriceDetail.slice(2) : null,
			...
			if (typeModal === "create") {
				...
				const result = obj?.mpricingDetail?.map((b) => {
					...
					const temp = {
						...
						flag: flag1,
						adjustment: b?.adjustment?.adjustmentText ? b?.adjustment?.adjustmentText : null,
						adjustmentId: b?.adjustment?.priceAdjustmentDetailId ? b?.adjustment?.priceAdjustmentDetailId : null
					};
					dataLength++;
					return temp;
				});
			...
			if (typeModal === "update") {
				...
				if (dataUpdate.priceCodeName === formValue.priceCode) {
					...
					if (
						...
						`${data[obj + 1]["priceCodeName"]}~${data[obj + 1]["min"]}~${data[obj + 1]["maximumName"]
						}`
					) {
	...
	const handleDelete = useCallback(
		(id, max, min) => {
			setData((prevState) =>
				prevState.filter((e) => `${e.priceCode}~${e.max}~${e.min}` !== `${id}~${max}~${min}`)
			);
		},
		[data]
	);
			...
			>
				<Form layout="vertical" form={formDetail} onFinish={handleAdd}>
					<div className="w-full grid grid-cols-2 gap-2">
						...
						<Form.Item label="Price Adjustment Detail" name="priceAdjustmentDetail">
							<InputComponent disabled />
						</Form.Item>
