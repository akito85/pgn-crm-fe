...
export const columns = (
	...
	RenderAction = () => { }
		...
		},
		{
			...
			render: (text) => renderColumn('saReference', hasValue(search['saReference']), searchText, text, false, 'input', search)
		},
		{
			title: "SERVICE TYPE",
			...
			render: (text) => renderColumn('saServiceType', hasValue(search['saServiceType']), searchText, text?.value, false, 'input', search)
		},
		{
			title: "TYPE",
			dataIndex: "saType",
			...
			render: (text) => renderColumn('saType', hasValue(search['saType']), searchText, text?.value, false, 'input', search)
		},
		{
			title: "PJBG TYPE",
			dataIndex: "pjbgType",
			...
			render: (text) => renderColumn('pjbgType', hasValue(search['pjbgType']), searchText, text?.value, false, 'input', search)
		},
		{
			title: "SA DATE",
			dataIndex: "saDate",
			...
			render: (text) => renderDateColumn('saDate', hasValue(search['saDate']), searchText, text, 'date', search)
		},
		{
			title: "START DATE",
			dataIndex: "startDate",
		...
		},
		{
			...
			render: (text) => renderDateColumn('commitmentDate', hasValue(search['commitmentDate']), searchText, text, 'date', search)
		...
		},
		{
			...
			render: (text) => renderColumn('billingCycle', hasValue(search['billingCycle']), searchText, text?.value, false, 'input', search)
		...
		},
		{
			...
			render: (text) => renderColumn('termsOfPaymentName', hasValue(search['termsOfPaymentName']), searchText, text?.value, false, 'input', search)
		},	
		{
			...
			render: (text) => renderColumn('invoiceTemplate', hasValue(search['invoiceTemplate']), searchText, text?.value, false, 'input', search)
		...
		},
		{
			...
			render: (text) => renderDateColumn('gasInPlanDate', hasValue(search['gasInPlanDate']), searchText, text, 'date', search)
		},
		{
			title: "STATUS",
			dataIndex: "status",
		...
		},
		{
			...
			render: (index) => {
				...
				return renderColumn('approvalStatus', hasValue(search['approvalStatus']), searchText, text, false, 'status', search)
			},
		},
		// {
		// 	title: "ACTION",
...
const ServiceAgreement = ({ idAccount, idCustomer, type }) => {
	...
	const filteredArray = {
		actionList: access_account?.actionList?.filter(action =>
			action.path.includes("/account-management/account-standard/service-agreement/") &&
	...
	useEffect(() => {
		dispatch(getGrantedAccessAccount('/account-management/account-standard/service-agreement'))
	}, [dispatch])
	...
	const handleSaveActivate = (e, handleClear) => {
		...
		dispatch(inactiveSa({ body: body, activeOrInactive: activeOrInactive }))
			...
			.then(() => {
				...
				const reqSearch = encodeURIComponent(JSON.stringify(search));
				dispatch(getListServiceAgreement({ id, search: reqSearch, sort, page, pageSize }));
	...
	const itemActions = [
		...
		},
		{
			...
			type: "table",
			render: (record, data) => {
				const isUpdate = record?.approvalStatus !== "WAITING APPROVAL" && record?.status !== "INACTIVE";
				const renderAction = data > 3 ? (
					isUpdate ? (
						<Link
							to={ACCOUNT_MANAGEMENT_ROUTES.UPDATE_SERVICE_AGREEMENT}
					...
					isUpdate ? (
						...
						<Tooltip title="Update">
							<div className="pt-1 cursor-not-allowed" >
								<SVGIcon name="IconEdit" className={'disabled cursor-not-allowed'} width={24} />
							</div>
						</Tooltip>
					)
				);
		...
		},
		{
			...
			type: "table",
			render: (record, data) => {
				const isEdit = record?.status === "ACTIVE" && (record?.approvalStatus === "APPROVED" || record?.approvalStatus === "REJECTED" || record?.approvalStatus === "DRAFT");
				const renderAction = data > 3 ? (
					isEdit ? (
						<ButtonComponent border={false}>
							<Checkbox
			...
			<Spin spinning={loading}>
				<BaseContainer header={"SERVICE AGREEMENT LIST"}>
					...
					<div className={"w-full"}>
						<TablePagination
							dataSource={data?.result?.map(item => ({ ...item, saServiceType: item?.serviceType, termsOfPaymentName: item?.termOfPayment }))}
