      ...
      >
        <CardComponent header={"ADDRESS INFORMATION"} cols={1}>
          ...
          <div className='grid grid-cols-4 gap-y-2.5'>
            ...
            <DetailText label="Province">{dataDetail?.province?.name}</DetailText>
            <DetailText label="City">{dataDetail?.city?.name}</DetailText>
            <DetailText label="District">{dataDetail?.district?.name}</DetailText>
          </div>
          <div className='grid grid-cols-4 gap-y-2.5'>
            <DetailText label="Sub District">{dataDetail?.subDistrict?.name}</DetailText>
            <DetailText label="Postal Code">{dataDetail?.postalCode?.name}</DetailText>
          ...
          <div className='grid grid-cols-4 gap-y-2.5'>
            ...
            <DetailText label="Street Name">{dataDetail?.streetName}</DetailText>
            <DetailText label="Block">{dataDetail?.block}</DetailText>
            <DetailText label="House Number">{dataDetail?.houseNumber}</DetailText>
          </div>
          <div className='grid grid-cols-4 gap-y-2.5'>
            ...
            <DetailText label="Additional Note">{dataDetail?.additionalNote}</DetailText>
          </div>
          <div className='grid grid-cols-1 gap-y-2.5'>
            <DetailText label="Address">{dataDetail?.fullAddress?.toUpperCase()}</DetailText>
          </div>
          <div className='grid grid-cols-1 gap-y-2.5'>
            <DetailText label="Description">{dataDetail?.descAddress}</DetailText>
          </div>
          ...
          <div className='grid grid-cols-4 gap-y-2.5'>
            ...
            <DetailText label="Premise">{dataDetail?.premiseFlag ? "Yes" : "No"}</DetailText>
            <DetailText label="Primary">{dataDetail?.primaryFlag ? "Yes" : "No"}</DetailText>
          </div>
          <div className='grid grid-cols-1 gap-y-2.5'>
            <DetailText label="Remark">{dataDetail?.descAccountAddress}</DetailText>
          </div>
        </CardComponent>
        <CardComponent header={"HISTORY LOG INFORMATION"} cols={5}>
          <DetailText label="Record ID">{dataDetail?.accountAddressId}</DetailText>
          <DetailText label="Created Date">{dataDetail?.cretedDate !== null ? moment(dataDetail?.createdDate).format(dateFormatting.dateTime) : ""}</DetailText>
