...
const ModalConfirmationAddress = ({
  ...
  const getCountryName = (val) => {
    const countryName =
      dataCountry && dataCountry?.data?.filter((item) => item?.id === val);
  ...
  const getProvinceName = (val) => {
    const provinceName =
      dataProvince && dataProvince?.data?.filter((item) => item?.id === val);
  ...
  const getCityName = (val) => {
    const cityName =
      dataCity && dataCity?.data?.filter((item) => item?.id === val);
  ...
  const getDistrictName = (val) => {
    const districtName =
      dataDistrict && dataDistrict?.data?.filter((item) => item?.id === val);
  ...
  const getSubDistrictName = (val) => {
    const subDistrictName =
      dataSubdistrict &&
      dataSubdistrict?.data?.filter((item) => item?.id === val);
  ...
  const getPostalCodeName = (val) => {
    const postalCodeName =
      dataPostalcode &&
      dataPostalcode?.data?.filter((item) => item?.id === val);
  ...
  const getBusiness = (val) => {
    const postalCodeName =
      dataBusiness && dataBusiness?.filter((item) => item?.id === val);
    if (postalCodeName === undefined) {
      return "";
    }
    if (postalCodeName.length !== 0) {
        ...
        <div>
          ...
          <div className={"w-full grid grid-cols-4 gap-5"}>
            <DetailText label={"Country"}>
              {getCountryName(dataToSend?.countryId)}
            </DetailText>
            <DetailText label={"Province"}>
              {getProvinceName(dataToSend?.provinceId)}
            </DetailText>
            <DetailText label={"City"}>
              {getCityName(dataToSend?.cityId)}{" "}
            </DetailText>
            <DetailText label={"District"}>
              {getDistrictName(dataToSend?.districtId)}
            </DetailText>
            <DetailText label={"Sub District"}>
              {getSubDistrictName(dataToSend?.subDistrictId)}
            </DetailText>
            <DetailText label={"Postal Code"}>
              {getPostalCodeName(dataToSend?.postalCodeId)}
            </DetailText>
          ...
          <div className={"w-full grid grid-cols-1 gap-5"}>
            <DetailText label={"Address"}>
              {(dataToSend?.address || "").toUpperCase()}
            </DetailText>
          </div>
          <div className={"w-full grid grid-cols-1 gap-5"}>
          ...
          <p className="text-primary uppercase font-bold pt-4">
            ADDRESS COORDINATE
          </p>
          <div className={"w-full grid grid-cols-4 gap-5"}>
          ...
          <div className={"w-full grid grid-cols-4 gap-5"}>
            <DetailText label={"Premise"}>
              {dataToSend?.premiseFlag ? "Yes" : "No"}
            </DetailText>
            <DetailText label={"Primary"}>
              {dataToSend?.primaryFlag ? "Yes" : "No"}
            </DetailText>
