...
const sectionKeys = ["tosInfo", "approval", "attachment"];
...
const ContentModalConfirm = ({
  ...
  const handleNextTab = () => {
    ...
    setActiveTab(sectionKeys[activeTabIndex + 1]);
  };
  ...
  const attachmentRows = useMemo(
    () =>
      (listDataAttachment || []).map((item, index) => ({
        key: item?.key || item?.id || index + 1,
        fileCategoryName: item?.fileCategoryName || item?.fileCategory || "-",
        fileName: item?.fileName || "-",
        createdBy: item?.createdBy || "-",
        createdDate: item?.createdDate || item?.uploadedDate || null,
  ...
  const attachmentColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        width: 200,
        sorter: (a, b) =>
          toLowerText(a?.fileCategoryName).localeCompare(toLowerText(b?.fileCategoryName)),
        ...getColumnSearchPropsUseFilteredValueFE(
          attachmentSearch,
          "fileCategoryName",
  ...
  return (
    <NxModal
      ...
      footer={
        [
          <div className="w-full flex justify-between gap-5" key="footer-confirm-tos">
    ...
    >
      <div className="p-4">
        <NxTabs
          ...
          {
            key: "tosInfo",
            label: "Term of Service Information",
            children: (
                ...
                <NxBaseContainer header="TERM OF SERVICE SUBMISSION INFORMATION" border>
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <DetailText label="Applied Date">
                      {dataTosSubmissionObj?.appliedDate
                        ? dataTosSubmissionObj.appliedDate.format(dateFormatting.date)
                        : dataTosSubmissionObj?.startDate
                          ? dataTosSubmissionObj.startDate.format(dateFormatting.date)
                          : "-"}
                    </DetailText>
                  ...
                  <div className="grid grid-cols-1 gap-4 mt-2">
                    <DetailText label="Description">{dataTosSubmissionObj?.remark || "-"}</DetailText>
                  </div>
                </NxBaseContainer>
          ...
          },
          {
            ...
            children: (
              <NxBaseContainer header="APPROVAL INFORMATION" border>
                ...
                <NxTable
                  idTable="tos-confirm-approval-table"
                  useSelect={false}
                  usePagination={false}
                  ...
                  expandable={{
                    ...
                    expandedRowRender: (record) => (
                      <NxTable
                        idTable={`tos-confirm-approval-employee-${record?.key || "row"}`}
                        useSelect={false}
                        usePagination={false}
                        showAdvanceSearch={false}
                        showSearchBar={false}
          ...
          },
          {
            ...
            children: (
              <NxBaseContainer header="ATTACHMENT INFORMATION" border>
                <NxTable
                  idTable="tos-confirm-attachment-table"
                  usePagination={false}
                  tableScrolled={{ x: "max-content", y: 320 }}
