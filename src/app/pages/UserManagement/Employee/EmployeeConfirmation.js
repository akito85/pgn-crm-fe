...
const EmployeeConfirmation = ({ data, data_emp }) => {
  ...
  const dataEmp = (value) => {
    return data_emp.find((item) => item.value === value)?.name;
  };
      ...
      <div className={"w-full flex flex-col gap-5"}>
        ...
        <div className={"w-full gap-5 "}>
          <div className={"w-full grid grid-cols-3 gap-5"}>
            <DetailText label={"Employee Number"}>{data?.empNumber}</DetailText>
          ...
          <div className={"w-full grid grid-cols-3 gap-5"}>
            <DetailText label={"Employee Type"}>
              {dataEmp(data?.empType)}
            </DetailText>
            <DetailText label={"Mobile Phone"}>{data?.phone}</DetailText>
        ...
        <div className="my-5">
          <TablePagination
            ...
            dataSource={data?.assignment?.map((item) => ({
              ...item,
