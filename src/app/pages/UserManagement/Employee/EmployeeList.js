...
import { getListEmpType, getListJob, getListPosition } from "../../../../redux/slices/user_management/employee";

const EmployeeList = ( props ) => {
  const { employeeList, onChangeEmployeeList = () => { }, assigmentEmployeeList, onChangeAssignmentEmployeeList = () => { } } = props;
  const { data_emp, data_post, data_job } = useSelector((state) => state.employee);
    ...
    <div className="bg-white w-full mt-[30px] p-[20px]">
      ...
      <div className="p-4">
        {segmentedPage === "EMPLOYEE" ? (
          <EmployeeUploadList
            dataEmp={employeeList} dataEmployeeType={dataEmployeeTypeMapped} onChangeData={onChangeEmployeeList}
          />
        ) : (
          <EmployeeAssigmentList
            dataAss={assigmentEmployeeList} dataJob={dataJobMapped} dataPosition={dataPositionMapped}  onChangeData={onChangeAssignmentEmployeeList}
          />
        )}
      </div>
    </div>
