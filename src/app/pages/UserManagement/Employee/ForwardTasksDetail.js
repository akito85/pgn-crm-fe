...
import { hasValue, renderColumn, renderDateColumn, renderDateConverter } from '../../../../utils';
...
const ForwardTasksDetail = ({ data }) => {
    ...
    const [searchedColumnAttacthment, setSearchedColumnAttacthment] = useState("");
    ...
    const columnDetail = [
        ...
        },
        {
            ...
            render: (text) => renderColumn('transactionId', searchedColumn, searchText, text, true, 'input', search)
        },
        {
            title: "APPROVAL TYPE",
        ...
        },
        {
            ...
            render: (text) => renderDateColumn('approvalDate', hasValue(search['approvalDate']), searchText, text, 'datetime', search)
        },
    ];
    ...
    return (
        <div>
            ...
            <CardComponent header={'Forward Task Information'} cols={4}>
                ...
                <DetailText label={"Forward Date"}>{hasValue(datas) && renderDateConverter(datas?.forwardDate, 'datetime')}</DetailText>
            </CardComponent>
            ...
            <CardComponent header={'Attachment Information'} cols={1} >
                <div className={"w-full flex flex-col"}>
                    <AttachmentComponent
                        type={"detail"}
                        data={datas?.attachmentTasks?.map(item => ({ ...item, urlFile1: item?.url1, urlFile2:item?.url2, dataType:'exist' }))}
