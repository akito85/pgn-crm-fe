    ...
    return (
        <BaseContainer header={'DATA ACCESS HIERARCHY INFORMATION'}>
            <div className={'w-full grid grid-cols-4'}>
                ...
                <DetailText label={"Start Date"}>{data?.startDate && moment(data?.startDate).format(dateFormatting.dateCapital)}</DetailText>
                <DetailText label={"End Date"}>{data?.endDate && moment(data?.endDate).format(dateFormatting.dateCapital)}</DetailText>
