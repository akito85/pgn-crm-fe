...
const ProfilePage = () => {
    ...
    const selectedEmployeeType = () => {
        ...
        if (data?.data?.authType?.toUpperCase() === "LOCAL") {
            ...
            if (actions?.includes('Update')) {
                filteredTabsEmployee = tabPagesEmployee.filter((page) => !(data?.data?.userType === "EMPLOYEE" && page.value === 'Change Profile'))
                tabPages = filteredTabsEmployee;
            } else {
                filteredTabsEmployee = tabPagesEmployee.filter((page) => (page.value === 'User Profile'))
                tabPages = filteredTabsEmployee;
            }
        } else {
    ...
    const layout = (valuePage) => {
        switch (valuePage) {
            case 'User Profile':
                return <UserInformationLayout data={data} header={'USER INFORMATION'} />
            case 'Change Profile':
        ...
        <LayoutMenu>
            <Spin spinning={loading}>
                ...
                <div className={'flex w-full gap-6'}>
                    ...
                    <BaseContainer type={'profile'} element={<RadioTabs data={selectedEmployeeType()} onChange={onChange} currentPosition={valuePage} />}>
