...
import { getProfile, removePicture } from '../../../../../redux/slices/user_management/profile';
                ...
                <div className={'w-full flex flex-col justify-center items-center gap-7'}>
                    {
                        data?.data?.urlImage2 === null ?
                            data?.data?.username === "" ?
                                ...
                                :
                                <Avatar size={180}>
                                    <span className={'text-[3.5rem]'}>{initialAvatar(data?.data?.username)}</span>
                                </Avatar>
                            :
                    ...
                    {actions?.includes('Upload') && (
                        <>
                            <ButtonComponent border={false} type={'submit'} onClick={handleOpen}>Upload Profile</ButtonComponent>
                            <span className={'text-[1rem] font-normal'}>
                                or
                            </span>
                            <ButtonComponent border={false} type={'default'} onClick={handleResetProfilePicture}>Reset Profile Picture</ButtonComponent>
                        </>
                    )
                    }
                </div>
            </BaseContainer>
            <ModalUpload openUpload={openModal} handleCancel={handleCancel} loading={loading} />
        </div>
    );
}
