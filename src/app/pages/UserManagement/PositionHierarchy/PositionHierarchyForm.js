...
const PositionHierarchyForm = (props) => {
  ...
  const { loading, data_position, data_detail, data } = useSelector(
  ...
  const assert = useCallback((data, data_position) => {
    ...
    if (data) {
      const transformDataTable = data?.rPhierarchys?.map((item, index) => ({
        ...
        positionName: hasValue(item?.positionId)
      ...
      setListPosition(
        data_position?.filter(
          (itemPosition) =>
            !data?.rPhierarchys?.some(
              (itemHierarchy) => itemHierarchy?.positionId === itemPosition?.id,
            ),
        ),
      );
      setListParentPosition(
        data_position?.filter((itemPosition) =>
          data?.rPhierarchys?.some(
            (itemHierarchy) => itemHierarchy?.positionId === itemPosition?.id,
          ),
        ),
  ...
  // assert row clicked
  const assertByRowClicked = useCallback(
    (data) => {
      if (data) {
        form.setFieldsValue({
          ...
          positionParent:
            data.positionParent === null || data.positionParent === undefined
              ? null
              : data.positionParent,
          remark: data.remark,
        });
  ...
  const handleDelete = useCallback(
    (record, dataParent) => {
      const findIndex = dataTable?.findIndex(
      ...
      if (record?.key === 1) {
        ...
        const filteredPosition = dataParent?.filter(
          (item) => item?.name === record?.positionName,
        );
        ...
        newData.forEach((item, index) => {
          if (
            index !== findIndex &&
            item.positionParent === record?.positionName
          ) {
  ...
  const handleClearInput = () => {
    setListParentPosition(
      data_position?.filter((itemPosition) =>
        dataTable?.some(
          (itemTable) => itemTable?.positionName === itemPosition?.name,
        ),
      ),
    );
    form.resetFields();
  ...
  const handleChangeListPosition = (value) => {
    const selectedPosition = listPosition?.filter(
  ...
  const handleUpdateRow = useCallback(
    (key) => {
      ...
      const selectedArrayFilter = dataTable?.filter(
        (item) => item?.key === key,
      );
      setSelectedPositionData(selectedArrayFilter);
      const hasChildren = dataTable?.filter(
        (item) => item?.positionParent === selectedArrayFilter[0]?.positionName,
      );
      ...
      setListParentPosition(
        data_position
          ?.filter((itemListParent) =>
            selectedArrayFilter?.some(
              (item) => item?.positionName !== itemListParent?.name,
            ),
          )
          ?.filter((itemTable) =>
            dataTable?.some((item) => item?.positionName === itemTable?.name),
          ),
      );
    },
  ...
  const onFinish = (formValue) => {
    ...
    if (updateByDiagramClicked) {
      ...
      if (id && data_detail && type === "update") {
        values = {
          key: maxId + 1,
          positionParent:
            formValue?.positionParent === undefined
              ? null
              : formValue?.positionParent,
    ...
    setListPosition(
      listPosition?.filter(
        (itemPosition) => itemPosition?.name !== formValue?.positionName,
      ),
    );
          ...
          return (
            <div className="w-full flex justify-center gap-4 mt-1 items-start">
              ...
              <Tooltip title={"Delete"}>
                <div
                  className={
                    updateByDiagramClicked || isDisabled
                      ? "cursor-not-allowed"
                      : "cursor-pointer"
                  }
                  onClick={() => {
                    updateByDiagramClicked === false &&
                      isDisabled === false &&
                      handleDelete(record, listParentPosition);
                  }}
                >
                  <DeleteOutlined
                    style={{
                      ...
                      fontSize: 24,
                      cursor:
                        isDisabled || updateByDiagramClicked
                          ? "not-allowed"
                          : "pointer",
                    }}
                  />
  ...
  const breadCrumbRoutes = [
    ...
    },
    {
      path:
        type === "update"
          ? USER_ROUTES.UPDATE_POSITION
          : USER_ROUTES.CREATE_POSITION,
  ...
  const isRequired = () => {
    let rules;
    if (
      selectedPositionData?.filter((item) => item?.key === 1)?.length === 1 &&
      updateByDiagramClicked === true
    ) {
      rules = false;
    ...
    if (type === "detail") {
      return (
        <DetailPosition
          ...
          data_detail={detailDiagram}
          data_position={data_position}
        />
      );
    } else {
      return (
        <ConfirmationHierarchy
          handleCancel={handleCancel}
          dataHierarchy={dataTable}
          handleSave={handleSaveHierarchy}
          id={id}
    ...
    <LayoutMenu>
        ...
        {type === "update" && (
          <BaseContainer header={"POSITION HIERARCHY INFORMATION"}>
            <div className={"h-[60px] w-full grid grid-cols-4 flex-wrap"}>
              <DetailText label={"Hierarchy Name"}>
                {data_detail?.name}
              </DetailText>
          ...
          <BaseContainer header={"POSITION HIERARCHY"}>
            <div className="grid grid-cols-2 gap-7">
              {!dataTable || dataTable?.length === 0 ? (
                ...
                <>
                  <PositionHierarchyDiagram
                    data={transformDataToTree(dataTable)}
              ...
              <div>
                ...
                >
                  <SelectComponent disabled={disabledChangeParent}>
      ...
      <ModalCustom
        ...
        footer={
          modalType === "detail" && (
            <div className={"w-full flex justify-end gap-5"}>
              <ButtonComponent type={"submit"} onClick={handleCancel}>
                Back
              </ButtonComponent>
            </div>
          )
