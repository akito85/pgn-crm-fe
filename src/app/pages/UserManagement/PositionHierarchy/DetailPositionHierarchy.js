...
import { getDetailHierarchy, getDetailPosition, getPosition } from '../../../../redux/slices/user_management/position_hirarchy';
...
const DetailPositionHierarchy = () => {
    ...
    const { data_detail, loading, data_position, data } = useSelector(state => state?.position_hierarchy);
    ...
    useEffect(() => {
        if (id && data_detail && data_position) {
            ...
            const transformDataTable = data_detail?.rPhierarchys?.map((item, index) => {
                return {
                    key: hasValue(item?.parentId) === false || item?.parentId === "" ? 1 : index + 2,
                    id: item?.id,
                    rHierId: item?.idHier,
                    parentId: hasValue(item?.parentId) && hasValue(idMap[item.parentId]) ? idMap[item.parentId] : null,
                    positionId: hasValue(item?.positionId) && hasValue(idMap[item.positionId]) ? idMap[item.positionId] : null,
        ...
        return (
            <g data-id={nodeDatum?.id} onClick={toggleNode}>
                ...
                <foreignObject {...foreignObjectProps}>
                    <div style={{ backgroundColor: 'lightblue', textAlign: 'center', borderRadius: '5px', }} onClick={() => handleDetailPosition(nodeDatum)} >
                                    ...
                                    <div className="flex-col items-center justify-center">
                                        ...
                                        <p>
                                            <Tooltip placement='top' title={
                                                nodeDatum?.positionName?.split(',')?.map(item =>
                                                (
                                                ...
                                                className='text-[12px]'>
                                                {nodeDatum.positionName?.split(", ")?.length} Employee(s)
                                            </Tooltip>
                                        </p>
                                    </div>
                                </div>
            ...
            <Spin spinning={loading}>
                <BaseContainer header={"POSITION HIERARCHY INFORMATION"}>
                    <div className={'w-full flex flex-col max-h-screen'}>
                        <div className={'h-1/6 w-full grid grid-cols-4 gap-10 flex-wrap mx-5'}>
                            <DetailText label={"Position"}>{data_detail?.name}</DetailText>
                            <DetailText label={'Status'}>{toTitleCase(data_detail?.status)}</DetailText>
                            <DetailText label={'Start Date'}>{hasValue(data_detail?.startDate) && data_detail?.startDate}</DetailText>
                            <DetailText label={'End Date'}>{hasValue(data_detail?.endDate) && data_detail?.endDate}</DetailText>
                        </div>
                ...
                <BaseContainer header={"HISTORY LOG  INFORMATION"}>
                    <div className={'w-full flex flex-col max-h-screen'}>
                        <div className={'h-1/6 w-full grid grid-cols-5 gap-10 flex-wrap mx-5'}>
                            <DetailText label={"Record Id"}>{
                                data_detail?.id}
                            </DetailText>
                            <DetailText label={"Created Date"}>{
                                hasValue(data_detail?.createdDate) && renderDateConverter(data_detail?.createdDate, 'datetime')}
                            </DetailText>
                            <DetailText label={'Created By'}>{data_detail?.createdBy}</DetailText>
                            <DetailText label={'Updated Date'}>
                                {hasValue(data_detail?.updatedDate) && renderDateConverter(data_detail?.updatedDate, 'datetime')}
                            </DetailText>
                ...
                <ModalCustom
                    ...
                    header={'ASSIGNMENT HISTORY'} width={980}
                    footer={
                    ...
                    <Spin spinning={loading}>
                        <DetailPosition data_detail={data} handleCancelModal={handleCancel} data_position={data_position} />
                    </Spin>
                </ModalCustom>
