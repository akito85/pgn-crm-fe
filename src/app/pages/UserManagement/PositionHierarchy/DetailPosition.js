...
const DetailPosition = ({ data_detail, handleCancelModal = () => { }, data_position }) => {
    ...
    const handleSetData = useCallback((data_detail, data_position) => {
        ...
        if (data_detail && hasValue(data_detail?.id)) {
            const transformData = {
                ...data_detail,
                positionId: hasValue(data_detail?.positionId) ? idMap[data_detail?.positionId] : null,
                parentId: hasValue(data_detail?.parentId) ? idMap[data_detail?.parentId] : null
            }
            ...
            {hasValue(data?.id) && (
                <>
                    <CardComponent header={'History Log Information'}>
                        <div className={'w-full grid grid-cols-5'}>
                            ...
                            <DetailText label={'Created Date'}>
                                {hasValue(data?.createdDate) && renderDateConverter(data?.createdDate, 'datetime')}
                            </DetailText>
                            <DetailText label={'Created By'}>
                                {data?.createdBy}
                            </DetailText>
                            <DetailText label={'Updated Date'}>
                                {hasValue(data?.updatedDate) && renderDateConverter(data?.updatedDate, 'datetime')}
                            </DetailText>
                    ...
                    <div>
                        ...
                        <TablePagination
                            dataSource={updatePagination(data?.employee, 'data', searchedColumn, searchText, page, pageSize, typeColumn)}
                            totalData={updatePagination(data?.employee, 'length', searchedColumn, searchText, page, pageSize, typeColumn)}
                            columns={column}
                            current={page}
                            pageSize={pageSize}
