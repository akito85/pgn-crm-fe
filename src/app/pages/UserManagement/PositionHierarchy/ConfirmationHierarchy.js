...
const ConfirmationHierarchy = ({
    ...
    const assertData = useCallback((data) => {
        if (data) {
            setInitStatus(data?.status)
            form.setFieldsValue({
                name: data?.name,
                saveAs: data?.status,
                startDate: hasValue(data?.startDate) === false || data?.startDate === "-" ? moment() : moment(data?.startDate).clone(dateFormatting?.date),
                endDate: hasValue(data?.endDate) === false || data?.endDate === "-" ? null : moment(data?.endDate).clone(dateFormatting?.date),
        ...
        if (id && data_detail) {
            const changeArray = dataHierarchy?.map(item => {
                ...
                return {
                    ...
                    parentId: hasValue(items?.positionParent) ? items?.positionParent : null,
