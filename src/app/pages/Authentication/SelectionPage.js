...
const SelectionPage = (props) => {
	const { type } = props;
	const { loading, token, positions, entities, remember } = useSelector((state) => state.auth);
		...
		<Spin spinning={loading}>
			<div className="h-screen w-full">
				<div className={'flex w-full bg-white px-5 pt-10 justify-between'}>
					<ButtonComponent onClick={() => setOpenModal(true)} icon={<LeftOutlined style={{ color: '#0075BF' }} type="default" />} border={false} >
						Log Out
					</ButtonComponent>
				...
				<div className={'w-full justify-center flex mt-10'}>
					<span className={' text-5xl text-[#0075BF]'}>
						{type === "position" ? "Choose Your Position" : "Choose Your Entity"}
					</span>
				</div>
				<CarouselListSelection type={type} data={data} remember={remember} setBody={setBody} />
				<ModalCustom
					isOpen={openModal}
