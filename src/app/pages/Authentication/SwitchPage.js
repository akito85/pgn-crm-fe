...
const SwitchPage = (props) => {
    const { type } = props;
    const { loading, token, user, data_entity, data_position, remember } = useSelector((state) => state.auth);
        ...
        <Spin spinning={loading}>
            <div className="h-screen w-full">
                <div className={'flex w-full bg-white px-5 pt-10 justify-between'}>
                    <ButtonComponent onClick={() => navigate(-1)} icon={<LeftOutlined style={{ color: '#0075BF' }} type="default" />} border={false} >
                        Back
                    </ButtonComponent>
                    <ButtonComponent type={'submit'}>
                        Welcome, {type === 'switch-entity' ? data_entity?.username : data_position?.username}
                    </ButtonComponent>
                </div>
                <div className={'w-full justify-center flex mt-10'}>
                    <span className={' text-5xl text-[#0075BF]'}>
                        {type === "switch-position" ? "Choose Your Position" : "Choose Your Entity"}
                    </span>
                </div>
                <CarouselListSelection type={type} data={type === 'switch-entity' ? data_entity?.listEntity : data_position?.listPosition} remember={JSON.parse(remember)} />
                <ModalCustom
                    isOpen={openModal}
