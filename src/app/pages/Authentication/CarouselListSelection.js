...
import { changeEntity, changePosition, chooseEntity, choosePosition, takeOverDelegation } from '../../../redux/slices/user_management/auth';
...
const CarouselListSelection = (props) => {
    ...
    const handleTakeOver = async (formvalue, handleCancel) => {
        try {
            const body = { ...formvalue, delegationId: record?.detail[0]?.delegationId, remember: record?.remember }
    ...
    // filter by type page
    const filterByTypePage = useCallback((type) => {
        let array = [];
        if ((type === "position" || type === 'switch-position') && data?.length > 1) {
            array = data.slice(0, 1);
            return array.map(item => ({ entityName: item?.positionName, entityId: item?.positionId, isDelegated: item?.isDelegated, ...item }))
        } else {
            array = data?.filter(item => (item?.entityName === "ALL"));
            return array;
        }
    }, [data])
    const filterData = useMemo(() => filterByTypePage(type), [filterByTypePage, type]);
                    ...
                    {index?.detail?.map((detail_index, key) => {
                        return (
                            <div className='my-5 w-full flex flex-col items-center'>
                                <div className={'w-5/6 border-solid border-b-[0.05rem] border-t-0 border-x-0  border-gray-300 flex justify-between px-2'} key={key}>
                                ...
                                <div className={'w-5/6 border-solid border-b-[0.05rem] border-t-0 border-x-0  border-gray-300 flex justify-between px-2'}>
                                    ...
                                    <span>{hasValue(detail_index?.start_date) && renderDateConverter(detail_index?.start_date, 'date')}</span>
                                </div>
                                <div className={'w-5/6 border-solid border-b-[0.05rem] border-t-0 border-x-0  border-gray-300 flex justify-between px-2'}>
                                    <span> End Date</span>
                                    <span>{hasValue(detail_index?.end_date) && renderDateConverter(detail_index?.end_date, 'date')}</span>
                                </div>
                            </div>
                        )
                    })}
                    ...
                    <div className='mt-5 flex flex-col w-full justify-center gap-3 items-center max-md:flex-row max-md:mt-0 max-md:px-3'>
                        {index?.isDelegated === true && (
                            <div className='max-md:w-1/2 w-1/3'>
                                <ButtonComponent type={'reject'} border={false} onClick={() => openTakeOver(index)} fullButton>
                                    Take Over
                                </ButtonComponent>
                            </div>
                        )}
        ...
        <div className={'w-full flex items-center justify-between mt-10 px-8'}>
            ...
            <div className="w-[90%] flex gap-2 justify-center">
                    ...
                    {filterData?.map((index, key) => (
                        <>
                            <RenderCards
                                ...
                                key={key}
                                type={type}
                                onClick={type === "position" || type === 'switch-position' ?
                ...
                <div
                    id="sliderTabAccount"
                    className={
                        `flex gap-2 max-h-max overflow-x-auto scroll whitespace-nowrap scroll-smooth no-scrollbar`
                    }
                >
                    {type === "position" || type === 'switch-position' ?
                        ...
                        : data?.filter(item => !(item?.entityName === "ALL"))?.map((index, key) => {
                            return (
                                <>
                                    <RenderCards
                                        name={index?.entityName}
