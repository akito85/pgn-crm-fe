...
const ListDetailWriteOff = () => {
  ...
  useEffect(() => {
    if (data_detail && data_detail.writeOff?.appHierId) {
      setSelectedHierarchy(data_detail.writeOff.appHierId);
      form.setFieldsValue({ apphierId: data_detail.writeOff.appHierId });
    }
    ...
    if (data_detail) {
      ...
      setDataHeader(data_detail.writeOff);
  ...
  const renderSection = (segmentedPage) => {
    switch (segmentedPage) {
      case "WriteOff":
        return (
          <>
            <DetailWriteOff data_detail={dataHeader} />
            <div className="mt-5">
      ...
      case "Approval":
        return (
          <div className="mt-5">
            <BaseContainer header={"WRITE OFF APPROVAL"}>
              <Form form={form}>
  ...
  const routes = [
    ...
    },
    {
      ...
      breadcrumbName: "Detail Write Off",
    },
  ];
  ...
  const handleConfirm = (res, handleClear) => {
    ...
    dispatch(approveOrRejectWriteOff({ body: data }));
    handleClear();
...
export default ListDetailWriteOff;
