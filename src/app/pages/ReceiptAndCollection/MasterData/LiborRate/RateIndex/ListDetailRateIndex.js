...
import DetailRateIndex from "./DetailRateIndex";
import {
  getDetailRateIndex,
...
const ListDetailRateIndex = () => {
  const dispatch = useDispatch();
  const location = useLocation();
  const navigate = useNavigate();
  const { id } = location?.state || {};
  const { data_detail, loading } = useSelector((state) => state.rateIndex);

  const [modalApprove, setModalApprove] = useState(false);
  const [approveOrReject, setApproveOrReject] = useState("");
  const [loadingConfirm, setLoadingConfirm] = useState(false);
  const [segmentedPage, setSegmentedPage] = useState("Rate Index");
  ...
  const routes = [
    ...
    { path: "", breadcrumbName: `Detail Rate Index` },
  ];
            ...
            items={[
              {
                label: "Rate Index",
                key: "Rate Index",
                children: <DetailRateIndex data={data_detail} />,
              },
              {
                label: "Attachment",
                key: "Attachment",
...
export default ListDetailRateIndex;
