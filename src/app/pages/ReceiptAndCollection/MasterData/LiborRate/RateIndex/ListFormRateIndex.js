...
import RateIndexForm from "./RateIndexForm";
import { ModalConfirm } from "../../../../../../components/Modal/ModalPopUp";
import BaseContainer from "../../../../../../components/BaseContainer";
import ModalCustom from "../../../../../../components/Modal/ModalCustom";
import ContentModalConfirmRateIndex from "./ContentModalConfirmRateIndex";
import {
  showModalError,
  showModalSuccess,
...
const ListFormRateIndex = (props) => {
  ...
  const {
    ...
    loading,
  } = useSelector((state) => state.rateIndex);

  const navigate = useNavigate();
  const dispatch = useDispatch();
  const [form] = Form.useForm();
  const location = useLocation();
  const { id, sourceId, sourceName } = location?.state || {};
  ...
  useEffect(() => {
    if (id && data_detail) {
      ...
      const formattedData = {
        ...
        startDate: rateIndex.startDate ? dayjs(rateIndex.startDate) : null,
        endDate: rateIndex.endDate ? dayjs(rateIndex.endDate) : null,
      };
  ...
  const [tabData] = useState([
    {
      value: "Rate Index", 
      paramValue: ["indexCode", "indexName", "currencyCode", "tenorValue", "tenorUnit", "ratePercentage", "startDate", "endDate"]
    },
  ...
  const handleSubmitForm = (formValue) => {
    ...
    const dataValue = {
      ...
      sourceId: sourceId || data_detail?.rateIndex?.sourceId,
      attachmentIds: listDataAttachment.filter(a => a.dataType === 'exist').map(a => a.id),
      startDate: formValue.startDate ? dayjs(formValue.startDate).format("YYYY-MM-DD") : null,
      endDate: formValue.endDate ? dayjs(formValue.endDate).format("YYYY-MM-DD") : null,
    };
  ...
  const routes = [
    ...
    { path: "", breadcrumbName: `Rate Index - ${sourceName || data_detail?.rateIndex?.rateSource?.sourceName || ''}` },
...
export default ListFormRateIndex;
