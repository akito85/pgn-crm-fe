...
const onFilter = (dataIndex, value, record) => {
  ...
  switch (dataIndex) {
    ...
    default:
      const val = record[dataIndex]?.label || record[dataIndex] || "";
      return val.toString().toLowerCase().includes(fixSearchText);
  }
};
...
const sorter = (fieldSort, a, b) => {
  const handleDataSort = (obj) => {
    switch (fieldSort) {
      ...
      default:
        const val = obj[fieldSort]?.label || obj[fieldSort] || "";
...
export const columnsTableCriteriaCollectionActivities = (
  listOption = {},
  searchInput,
  searchedColumn,
  searchText,
